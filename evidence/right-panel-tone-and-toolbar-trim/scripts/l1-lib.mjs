...
// (right-panel-tone-and-toolbar-trim: daihon-placed-text-bars から複製。launch 失敗時に自分の Electron を止める try を追加)
...
export async function launch({ shellDir, electron, project, port, isoDir }) {
    ...
    } catch (error) { cdp.close(); try { process.kill(pid, 'SIGKILL'); } catch {} throw error; }
    return { pid, cdp };
}
