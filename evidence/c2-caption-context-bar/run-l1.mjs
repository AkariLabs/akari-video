...
const projectName = 'c2';
const only = (argument('only', '') || '').split(',').filter(Boolean);
const theme = argument('theme', 'dark');
const port = Number(process.env.AKARI_CDP_PORT ?? 9578);
const electron = path.join(shellDir, 'node_modules', 'electron', 'dist', 'Electron.app', 'Contents', 'MacOS', 'Electron');

const scratch = await realpath(await mkdtemp(path.join(os.tmpdir(), `2026-09-26-libcanvas-c2-caption-context-bar-${label}-`)));
...
await copyFile(path.join(here, 'fixtures', 'c2.json'), editPath);
await copyFile(path.join(here, 'fixtures', 'c2.captions.json'), path.join(project, 'captions.json'));
...
try {
  ...
  if (view) {
    ...
    await sleep(1500);
    outer = await frameOffset();
    const api = { captionsPath, main, browser, get view() { return view; }, get outer() { return outer; }, editPath, editUri, outDir, label, S,
    ...
    if (argument('hold')) { await writeFile('/tmp/2026-09-26-libcanvas-c2-caption-context-bar-hold.json', JSON.stringify({ view, outer, editPath })); await sleep(Number(argument('hold')) * 1000); }
