#!/usr/bin/env node
// L1（検証専用・ラッパー作成）: インスペクターのスクロールの保持と、プレビュー ⇄ インスペクターのライブの連動を
// 本物の Electron シェルで確かめる。
...
// 使い方: node evidence/kf2-inspector-live/run-l1.mjs --shell <apps/shell> --out <dir> --label before|after [--only a,b]
...
const outDir = path.resolve(arg('out', path.join(repo, 'evidence/kf2-inspector-live', label)));
const only = (arg('only', '') || '').split(',').filter(Boolean);
const port = Number(process.env.AKARI_CDP_PORT ?? 9572);
const httpPort = Number(process.env.AKARI_HTTP_PORT ?? 48971);
const electron = path.join(shellDir, 'node_modules/electron/dist/Electron.app/Contents/MacOS/Electron');
const ffmpeg = process.env.FFMPEG ?? 'ffmpeg';

const scratch = await realpath(await mkdtemp('/tmp/2026-09-26-libcanvas-kf2-inspector-live-l1-'));
...
const EDIT = {
  ...
  tracks: [
    ...
    { id: 'v-box-a', lane: 'visual', name: 'box-a', items: [{ id: 'box-a', at: 0, duration: 300, transform: { x: 300, y: 200 },
      source: { kind: 'shape', shape: 'rect', params: { width: 300, height: 200, fill: '#3b82f6' } } }] },
...
const CAPTIONS = { captions: [{ id: 'c-0001', start: 0, end: 10, time_domain: 'output', text: 'ライブの連動の確かめ', speaker: null, sourceRef: null, edited: true }] };
