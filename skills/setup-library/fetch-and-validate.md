...
配置前に**登録先スコープ**をユーザーに確認する（契約「アセットのスコープ階層」参照）: `user`（`~/.akari/assets/`、全プロジェクト共通 — スターターパックの既定候補）/ `local`（プロジェクト内 `assets/`）/ `shared`（上位ディレクトリの `.akari/assets/`）/ `builtin`（製品リポの `assets/` — 開発者がライブラリ自体を育てる場合のみ）。以降の `assets/...` 表記は選ばれた層のルートを指す。
