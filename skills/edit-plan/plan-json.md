...
録画素材がある場合は plan.json を作らず、従来どおり分析統合からレポートへ進む
...
- 対話サーフェスは [decision-cards](../../packages/decision-cards/) の既存機構を
  ...
  ヘルパー不通時はチャットの明示承認で代替し、その旨を記録する（レポートの Checkpoint
  運用と同じ代替規約）
- 編集判断レポートの decisions.json とはファイルを分ける。混ぜない

## 2. plan.json を書く

- 置き場所は `<plan-dir>/plan.json`（レポートと同じディレクトリ。AKARI プロジェクトでは
  `planning/` ロールが既定）
...
- 同一ファイルを段階的に更新する（レポート HTML と同じ規律）。決定の経緯は対話の
  decisions.json が持つため、plan.json 側には結果だけを書く
- 書いたら毎回検証する:
...
- 以降のレポート工程（[report-guide.md](report-guide.md)）では、分析サマリ・編集方針・
  素材計画の各章が plan.json の slot id を根拠として参照する。台本や構成をレポート側で
  再フリーテキスト化しない
- BGM・SFX・B ロール等の素材三択（提案/生成/不採用）は従来どおりレポート §5 素材計画の
  管轄。plan.json には持ち込まない（契約 §5）

## 4. 仮枠プレビュー（現時点の運用）

仮枠の再生 QA は「静止画 + TTS + テキストカードを通常の edit.json v1 にコンパイルする」
規約（契約 §6）で行う設計だが、**コンパイラは未実装**。当面は静止コンセプトをレポートへ
埋める従来運用（workflow.md §2）を維持し、動画生成は承認工程まで保留する。
コンパイラ実装後にこの節を差し替える。
