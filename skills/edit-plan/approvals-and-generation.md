...
選択が人間の意図や費用を変える場合は、宣言だけで実行せず回答を待つ。契約ですでに静止画の作成が承認されている場合も、手と理由は宣言して `decision_log` に追記する。失敗時に別の手へ切り替える前も、変更理由と得失を追記する。
...
生成物ごとに、レポートと `decision_log` から次を辿れるようにする。

- asset ID と種別
- 使用した手と、分かる場合は model / tool version
- prompt と参照素材
- ISO 8601 の生成日時
- 出力ファイルとレポート内 preview
...
契約上の checkpoint 数 `3` は M5 の確定値であり不変。カード化は Checkpoint 1（方針）の**回答チャネルの追加**であって、段数を増減しない。Checkpoint 2（素材計画）/ 3（実行 manifest）は v0 では従来どおり会話チャネルで停止・明示承認を待つ（カード化は次段）。

### Checkpoint 1: 方針

サムネイル、分析、編集方針、カット案を提示し、選択肢の得失を説明する。人間から今回の方針に対する明示回答を得たら追記し、修正ならレポートを更新する。承認前は素材計画を確定しない。

**明示回答のチャネルは 2 本**（出口は 1 つ）:

1. **決定カードの commit**（既定経路）: レポート HTML の決定カードを人間が確認し、要約ビュー（`data-view="summary"`）を経て確定すると、ヘルパーが `<レポートパス>.decisions.json` の `completedAt` を **非 null** にする。これが今回の方針に対する明示回答となる。エージェントは確定内容（`byDefault: false` に変わった差分。おまかせ確定なら「全カード AI 推奨のまま」）を `decision_log` に追記する。
2. **チャットでの回答**（劣化経路・引き続き有効）: ヘルパーが不通、または decisions.json が無い・破損している場合は、会話上の明示承認で代替できる。その旨（劣化経路を採った理由）を `decision_log` に記録する。

**編集実行ゲート**: `completedAt` が入るか、チャットで明示承認を得るまで、編集実行（素材計画の確定・[execution.md](execution.md)）に進まない。無操作・タイムアウト・過去の包括承認を今回の承認に読み替えない。カードのローカル表示上のチェックだけでは承認とみなさない。
