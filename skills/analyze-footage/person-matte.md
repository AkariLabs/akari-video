...
`{"available":true}` 以外なら `reason` を報告し、マットを作らずに `null` のまま進む。macOS、
...
- `--quality fast|balanced|accurate`（既定 `balanced`）
...
成功時は 1 行の JSON が返る。`ok`、`frames`、`bytes`、`elapsed_seconds`、`vision_ms_per_frame`、
