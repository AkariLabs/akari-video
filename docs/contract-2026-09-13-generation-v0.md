...
```json
{
  ...
  "next": {
    ...
    "inputs": { "prompt": "…", "first_frame": { "path": "…", "sha256": "…" }, "last_frame": null,
                "reference_images": [], "reference_videos": [], "reference_audios": [],
                "camera": null, "seed": null, "extra": {}, "mode": "frames" },
...
3. **`inputs.mode`** = `"frames"` / `"references"`。最初 / 最後と参照が排他のとき（`frames_and_refs_exclusive: true` の行、または同じ family の i2v 行と ref 行の切替）、**下書きは両側を保持し、送るのは `mode` の側だけ**。切り替えで中身を消さない。バリデータは `mode` を見て反対側を送信 body から外す
