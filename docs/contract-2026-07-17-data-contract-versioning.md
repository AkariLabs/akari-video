...
### 2026-08-21 追補 — v2 audio tracks への最終構造変更

task `2026-08-20-v2-audio-tracks` に限り、「機能追加禁止・バグ修正のみ」の明示承認済み・1 回限りの例外とする。これはバグ修正ではなく、ffprobe による実尺解決と `tracks[].items[]` への音声出力という構造的能力を追加する変更である。ただし、音声を `tracks[]` 外の秒ベース block として残した設計自体が親タスクの解消対象だったため許可する。製品版 `1.0.0` または `2026-12-31` の早い方で終了する変換器に対して、これを最後に許可された構造変更とし、以後の拡張は認めない。
