...
- 発端: オーナー実機フィードバック 2026-07-25 — 実プロジェクト（selection-dogfood）の
  ルートに `frame-*.png` 9 枚（キーフレーム視認証跡）・`negative-rule-check.mjs` +
  `-result.json`（エージェントの ad-hoc 検証）・`final.mp4` が散乱。
  「もう全部構造化できないと困る」。競合 HyperFrames は paths 宣言 + 隠し領域隔離 +
  スキルが場所を教える 3 本柱で統制しており、その AKARI 版を契約化する
...
競合 HyperFrames Studio の 3 本柱（paths 宣言・隠し領域隔離・スキルが場所を教える）のうち、
「隠し領域隔離」と「スキルが場所を教える」を輸入する。「paths 宣言」（プロジェクトごとの
...
発端（selection-dogfood の実機フィードバック）で観測された散乱ファイル 3 種は、本契約下では
それぞれ次の層へ行く。
...
- HyperFrames の「paths 宣言」（プロジェクトごとの置き場所カスタマイズ UI）の輸入
