...
`speech` は cuts[] の撮影素材に含まれる音声を表し、各カットの trim / speed / freeze を
出力タイムラインへ投影して Web Audio プレビューへ供給する。ただし `speech` は ducking の
側鎖へ入れない。書き出しの `sidechaincompress` で鍵入力になるのは narration のみであり、
土台カット音声 `[0:a]` は鍵ではないため、プレビューも同じ対象区間を維持する。
