...
**Q. 完全自動で最後まで走らせられる？**
既定の**提案つき**（`checkpoint`）では途中の承認はなく、書き出しが唯一の判子です。
`full-auto`（そのまま）は書き出しも見ずに行い、`collaborative`（一緒に作る）は
方針 → 素材計画 → 実行の 3 段階で確認します。
**課金と外部送信**の承認はこれと独立で、`connections.json` のコスト承認ポリシーが優先されます。
