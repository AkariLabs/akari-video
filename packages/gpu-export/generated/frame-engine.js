...
`,
  ...
  function isForceSoftwareDecode() {
  ...
  // packages/frame-engine/src/decode/source-bytes.ts
      ...
      if (decoder.decodeQueueSize === 0) {
        ...
        try {
          return await Promise.race([
            waiter.promise.then(() => "target-or-dequeue"),
            graceExpired,
            this.decoderFailure ?? new Promise(() => void 0)
          ]);
        } catch (error) {
          throw error instanceof DecoderExecutionError ? error : new DecoderExecutionError(`decoder output grace failed for target ${waiter.targetUs}us`, error);
        } finally {
  ...
  function createPreviewAudioSupply(options) {
    ...
    const startFrom = async (seconds) => {
      ...
      const plan = scheduleBuilder({
        timelineDurationSec,
        startAtSec: clamp4(latestRequestedSec || seconds),
  ...
  var SpriteCompositor = class {
    ...
    instanceVertexArray;
    instanceBuffer;
    instanceCanvasLocation;
    plainInstanceProgram;
    plainInstanceVertexArray;
    plainInstanceBuffer;
    textureUnitCount;
    baseTexture;
    sprites = /* @__PURE__ */ new Map();
    instanceCapacity = 1;
    instanceData = new Float32Array(TILE_INSTANCE_FLOATS);
    plainInstanceCapacity = 1;
    plainInstanceData = new Float32Array(PLAIN_INSTANCE_FLOATS);
    probe = null;
    disposed = false;
    constructor(canvas, options = {}) {
      ...
      const instanceVertex = compileShader2(gl, gl.VERTEX_SHADER, `#version 300 es
      ...
      const samplerDeclarations = Array.from(
        ...
        (_3, index) => `uniform sampler2D uTexture${index};`
      ).join("\n");
      const samplerBranches = Array.from(
        { length: this.textureUnitCount },
        (_3, index) => `if (unit == ${index}) return texture(uTexture${index}, coord);`
      ).join("\n");
      const instanceFragment = compileShader2(gl, gl.FRAGMENT_SHADER, `#version 300 es
      precision highp float;
      in vec2 uv;
      flat in float vMix;
      ...
      const plainInstanceFragment = compileShader2(gl, gl.FRAGMENT_SHADER, `#version 300 es
      precision highp float;
      in vec2 uv;
      flat in float vOpacity;
    ...
    buildInstanceRuns(draws) {
      const runs = [];
      let run = null;
      for (const draw of draws) {
        const kind = draw.tiles === void 0 && draw.textureRect === void 0 ? "plain" : "tile";
        ...
        let highlight = base;
        if (draw.tiles !== void 0 && draw.secondaryId !== void 0) {
    ...
    drawInstanceRuns(runs, state) {
      const gl = this.gl;
      const chunks = this.planRunChunks(runs, state);
      const tileCount = chunks.filter((chunk) => chunk.kind === "tile").reduce((sum, chunk) => sum + chunk.count, 0);
      const plainCount = chunks.filter((chunk) => chunk.kind === "plain").reduce((sum, chunk) => sum + chunk.count, 0);
    ...
    planRunChunks(runs, state) {
      ...
      for (const run of runs) {
        ...
        for (const instance of run.instances) {
          const required = run.kind === "plain" ? [instance.texture] : instance.highlight === instance.base ? [instance.base] : [instance.base, instance.highlight];
          const missing = required.filter((texture) => !current?.textures.includes(texture));
          if (current && current.instances.length > 0 && current.textures.length + missing.length > this.textureUnitCount) {
    ...
    ensurePlainInstanceCapacity(count) {
