...
test("(c) 静的スプライトは overlay の transform を落とさない", () => {
  ...
  assert.match(spriteRoot, /transform:translate\(var\(--x, 0px\), var\(--y, 0px\)\) rotate\(var\(--rotate, 0deg\)\) scale\(var\(--scale-x, var\(--scale, 1\)\), var\(--scale-y, var\(--scale, 1\)\)\);transform-origin:center;/u);
  // OSR のコンテナと同じ宣言であること
  ...
  assert.match(axisSheet, /\.akari-overlay-container \{[^}]*transform: translate\(var\(--x, 0px\), var\(--y, 0px\)\) rotate\(var\(--rotate, 0deg\)\) scale\(var\(--scale-x, var\(--scale, 1\)\), var\(--scale-y, var\(--scale, 1\)\)\); transform-origin: center;/u);
