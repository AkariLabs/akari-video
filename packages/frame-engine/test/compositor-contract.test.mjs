...
function layerFragment() {
  ...
  const gl = new Proxy({}, {
    get(_target, key) {
      if (key === 'shaderSource') return (_shader, text) => shaders.push(text);
  ...
  const compositor = new WebGL2Compositor({ getContext: () => gl });
  compositor.dispose();
...
test('grain uses an integer frame hash and vignette uses the crop-local box', () => {
  ...
  assert.match(fragment, /box \/ max\(min\(box\.x, box\.y\)/u);
  assert.match(fragment, /params\.w == 0\.0 \? step/u);
  assert.match(fragment, /rgb - vec3\(params\.x \* falloff\)/u);
});
