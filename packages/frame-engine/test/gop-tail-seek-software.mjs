// CI（Linux xvfb）の HW デコード不可条件を mac で再現する L1。
// `npm run test:seek` と同じ検査を software デコードだけで通す。
...
const resultsName = 'gop-tail-seek-software-results.json';
const resultsPath = resolve(generated, resultsName);
...
const environment = { ...process.env, AKARI_SEEK_RESULTS_NAME: resultsName };
delete environment.ELECTRON_RUN_AS_NODE;
if (existsSync(resultsPath)) unlinkSync(resultsPath);
const execution = spawnSync(electron, ['--no-sandbox', resolve(testDirectory, 'gop-tail-seek-software-main.cjs')], {
  cwd: packageDirectory,
  encoding: 'utf8',
  timeout: 330_000,
  env: environment,
...
assert.equal(existsSync(resultsPath), true, 'software seek renderer did not write results');
