...
test('a direct move completes the media point and reevaluates to the visible target', () => {
  ...
  assert.deepEqual(saved.keyframes[1].transform,
    { ...base, scale: .25, scaleX: .25, scaleY: .25, rotate: 8 });
  assert.equal(saved.keyframes[0].transform.x, -200);
  assert.equal(saved.keyframes[2].transform.x, 200);
