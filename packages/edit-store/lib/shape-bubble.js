...
function bodyPoints(A, B, p) {
    ...
    if (st === 'rect' || st === 'round') {
        ...
        const line = (x0, y0, x1, y1) => {
            ...
            for (let i = 0; i < k; i++)
                P.push([x0 + (x1 - x0) * i / k, y0 + (y1 - y0) * i / k]);
        };
        const arc = (cx, cy, a0) => {
            if (!r)
                return;
...
function bubbleGeometry(A, B, p) {
    ...
    if (tail === 'none')
        ...
        const base = [dx * rp, dy * rp]; // 本体の縁の、しっぽの向きの点
        if (tail === 'point') {
            ...
            const Qb = [ctl[0] + (Pb[0] - R0[0]) * .35, ctl[1] + (Pb[1] - R0[1]) * .35];
            const quad = (a, q, b) => {
                const o = [];
                for (let j = 1; j < 18; j++) {
                    const t = j / 18;
...
const serializeBubble = (g, sx, sy, ox, oy) => g.subs.map((s) => 'M' + s.map((q) => (+((q[0] - g.x0) * sx + ox).toFixed(2)) + ' ' + (+((q[1] - g.y0) * sy + oy).toFixed(2))).join('L') + 'Z').join('');
