...
function serializeShapePath(subs) {
    return subs.map((s) => `M${point(s.start)}` + s.segs.map((g) => g.t === 'L' ? `L${point(g.p)}` : `C${point(g.c1)} ${point(g.c2)} ${point(g.p)}`).join('') + (s.closed ? 'Z' : '')).join('');
}
function fitShapePath(d, width, height) {
    ...
    return subs.map((s) => ({
        start: map(s.start),
        closed: s.closed,
        segs: s.segs.map((g) => g.t === 'L'
            ? { t: 'L', p: map(g.p) }
            : { t: 'C', c1: map(g.c1), c2: map(g.c2), p: map(g.p) }),
    }));
}
