 ...
 */
export function captionFragmentWindows(caption: CaptionFragmentLike): CaptionFragmentWindow[] | null {
    ...
    const validWords = words?.every(word => isCaptionFragmentWord(word)) === true
        ? words as Array<{ text: string; start: number; end: number }> : null;
    const wordText = validWords?.map(word => word.text).join('');
    const fragmentEnds: number[] = [];
...
export function expandCaptionDisplayFragments<T extends CaptionFragmentLike>(captions: readonly T[]): Array<T & {
    ...
    return captions.flatMap(caption => {
        ...
        return windows.map(window => {
            ...
            const expanded: CaptionFragmentLike = {
                ...caption,
                text: window.text,
                start: window.start,
                end: window.end,
            ...
            characterStart = characterEnd;
            return expanded as T & { fragmentIndex: number; fragmentCount: number; fragmentKey: string };
        });
    });
}

function isCaptionFragmentWord(value: unknown): value is { text: string; start: number; end: number; [key: string]: unknown } {
