...
export function setCaptionTimingLine(
    source: string,
    captionId: string,
    start: number,
    end: number,
    timeDomain: 'source' | 'output' | null | undefined,
    edited: boolean
): string {
    ...
    nextElement = updateOptionalStyleProperty(
        nextElement,
        'time_domain',
        timeDomain,
        `字幕 ${captionId}`
...
export function updateCaptionFieldsInSourceWithReport(
    source: string,
    captionId: string,
    updates: {
        text?: string;
    ...
    if (updates.unrecognized !== undefined) {
        nextElement = syncOptionalCaptionProperty(
            nextElement,
            'unrecognized',
            updates.unrecognized === null || unrecognized?.length === 0 ? undefined : unrecognized,
            captionId
        );
    }
...
function shadowedPresetStyleKeys(presetId: string, textStyle: unknown): string[] {
    ...
    const style = textStyle as Record<string, unknown>;
...
function pruneShadowedTextStyle(element: string, keys: readonly string[], captionId: string): string {
    if (keys.length === 0) {
        return element;
    }
    ...
    return Object.keys(JSON.parse(textStyle) as Record<string, unknown>).length === 0
...
export function splitCaptionLine(
    source: string, captionId: string, wordIndex: number, newCaptionId: string
): string {
    const array = locateCaptionArray(source);
    const entries = captionElementEntries(array.elements);
    ...
    const recordB: Record<string, unknown> = {
    ...
    if (Array.isArray(record.runs)) {
        ...
        const runs = rebaseCaptionRuns(displayText, rawText, record.runs as CaptionRun[]).runs;
        const runsA = sliceCaptionRuns(rawText, runs, 0, split);
        const runsB = sliceCaptionRuns(rawText, runs, split, rawText.length);
        if (runsA) recordA.runs = runsA;
        else delete recordA.runs;
        if (runsB) recordB.runs = runsB;
        else delete recordB.runs;
    }
    ...
    for (const output of [recordA, recordB]) {
        ...
        if (Array.isArray(output.words) && output.words.length === 0) delete output.words;
        if (Array.isArray(output.unrecognized) && output.unrecognized.length === 0) delete output.unrecognized;
    }
...
export function mergeCaptionLines(source: string, captionIds: readonly string[]): string {
    ...
    if (records.some(record => Array.isArray(record.runs))) {
        let offset = 0;
        const mergedRuns = joinAdjacentCaptionRuns(records.flatMap(record => {
            const rawText = String(record.text ?? '');
            const displayText = typeof record.display_text === 'string' ? record.display_text : rawText;
            const runs = Array.isArray(record.runs)
                ? rebaseCaptionRuns(displayText, rawText, record.runs as CaptionRun[]).runs : [];
            const projected = runs.map(run => ({ ...run, from: run.from + offset, to: run.to + offset }));
            offset += captionGraphemes(rawText).length;
            return projected;
        }));
        if (mergedRuns.length > 0) survivor.runs = mergedRuns;
        else delete survivor.runs;
    }
    delete survivor.display_text;
    delete survivor.display_fragments;
    if (words.length === 0) delete survivor.words;
    if (unrecognized.length === 0) delete survivor.unrecognized;

    const selected = new Set(elements);
    const survivorElement = elements[0];
    const kept = array.elements.flatMap((element, index) => {
        if (!selected.has(element)) return [{ element, index, text: element.text }];
...
function serializeCaptionRaw(value: Record<string, unknown>): string {
    const schemaKeys = [
        'id', 'start', 'end', 'text', 'speaker', 'sourceRef', 'edited', 'src',
        'time_domain', 'words', 'unrecognized', 'style', 'display_text',
        'display_fragments', 'style_preset', 'text_style', 'display_timing', 'runs'
    ...
    for (const key of schemaKeys) {
        if (value[key] !== undefined && (key !== 'runs' || Array.isArray(value[key]) && (value[key] as unknown[]).length > 0)) {
            parts.push(`${JSON.stringify(key)}: ${JSON.stringify(value[key])}`);
        }
    }
    for (const [key, item] of Object.entries(value)) {
        if (!known.has(key) && item !== undefined) parts.push(`${JSON.stringify(key)}: ${JSON.stringify(item)}`);
    }
    return `{ ${parts.join(', ')} }`;
}
...
function normalizeUnrecognized(value: unknown): { start: number; end: number }[] | undefined {
    ...
    const spans = value.map(span => {
        const record = span as Record<string, number>;
...
function validateTextStylePatch(updates: CaptionTextStylePatch): void {
    ...
    for (const [name, effect, fields] of [
        ...
        ['glow', updates.glow, ['density', 'spread'] as const]
    ] as const) {
...
function shadowPatchToJson(shadow: CaptionShadow): Record<string, unknown> {
    return {
        color: shadow.color,
...
function updateOptionalObjectStyleProperty(
    source: string,
    property: 'shadow' | 'glow',
    value: CaptionShadow | CaptionGlow | null | undefined,
    label: string
): string {
    if (value === undefined) return source;
    const existing = locateTopLevelProperty(source, property);
    if (value === null) return existing ? removeObjectProperty(source, property) : source;
    const json = property === 'shadow'
        ? shadowPatchToJson(value as CaptionShadow) : glowPatchToJson(value as CaptionGlow);
    if (!existing) return appendJsonProperty(source, property, json);
...
function updateAnimationStyleObject(
    source: string,
    updates: CaptionTextStylePatch['animation'],
    label: string
): string {
    if (updates === undefined) return source;
    const located = locateTopLevelProperty(source, 'animation');
    if (updates === null) return located ? removeObjectProperty(source, 'animation') : source;
    if (!located) {
        const created = {
    ...
    for (const [slotName, slot] of [['in', updates.in], ['out', updates.out]] as const) {
        ...
        const replaced = slotProperty.text.replace(pattern, (_match, prefix, suffix) =>
