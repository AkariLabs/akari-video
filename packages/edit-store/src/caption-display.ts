...
export function resolveCaptionDisplay(
    ...
    const fragmentsByCaption = new Map<number, {
        fragments: string[];
        manual: boolean;
        overflow?: CaptionDisplayOverflow;
    }>();
    captions.forEach((caption, index) => {
        ...
        let manual = false;
        let overflow: CaptionDisplayOverflow | undefined;
        if (!projected.changed && caption.display_fragments !== undefined) {
            ...
            if (split.overflow) overflow = { ...split.overflow, units: policy.max_line_units };
    ...
    const previousByGroup = new Map<string, CaptionDisplayCue>();
    for (const cue of displayCues) {
        ...
        if (!previous || cue.end > previous.end) previousByGroup.set(group, cue);
    }
    return {
...
function validateManualFragments(
    caption: UnknownRecord,
    text: string,
    policy: CaptionDisplayPolicy
): { fragments: string[]; overflow?: CaptionDisplayOverflow } {
    const overflow = (): { fragments: string[]; overflow: CaptionDisplayOverflow } => ({
        fragments: [text],
...
export function splitCaptionFragments(
    text: string,
    policy: CaptionDisplayPolicy
): { fragments: string[]; boundaries: number[]; overflow?: CaptionDisplayOverflow } {
...
function splitCaptionFragmentsAtBoundaries(
    ...
    const visit = (start: number, remaining: number): string[] | undefined => {
        ...
        for (const end of candidates) {
            ...
            if (rest) return [text.slice(start, end), ...rest];
        }
        return undefined;
    };
    for (let count = 3; count <= maximumFragments; count++) {
        const fragments = visit(0, count);
        if (fragments && fragments.length <= count) return fragments;
    }
    return undefined;
}
...
function normalizeCaptionAnimationSlot(value: unknown): UnknownRecord | undefined {
...
function normalizeCaptionLineTextStyle(value: unknown): UnknownRecord {
...
export function mergeCaptionLineTextStyles(base: unknown, override: unknown): UnknownRecord | null {
...
export function usesExtendedPerLineBackground(background: unknown): boolean {
    if (!isRecord(background) || background.mode === 'block') return false;
...
function captionZoneVars(zone: unknown): Record<string, string> {
...
function resolveCaptionLineStyleVarsAtScale(style: UnknownRecord, scale: number): Record<string, string> {
    ...
    if (isRecord(style.background) && (typeof style.background.color === 'string' || finiteNumber(style.background.opacity))) {
        ...
        vars[name] = colorWithOpacity(
            ...
            finiteNumber(style.background.opacity) ? style.background.opacity : undefined
        );
    }
...
export function resolveCaptionLineStyleVars(
    style: UnknownRecord | null | undefined,
    output: { width: number; height: number } | undefined
): Record<string, string> {
...
export function resolveCaptionStyleForOutput(style: UnknownRecord, output: { width: number; height: number } | undefined): { vars: Record<string, string>; layout?: ResolvedCaptionLayout } {
    ...
    if (layout === undefined) {
        ...
        for (const name of Object.keys(captionZoneVars(style.zone))) delete vars[name];
