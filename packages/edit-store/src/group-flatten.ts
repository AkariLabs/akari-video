...
export function flattenGroupDescendants(internal: InternalEdit): FlattenedVisualItem[] {
    ...
    const visit = (item: InternalItem, track: InternalTrack, parent?: Context, descendant = false): void => {
        ...
        const transform = composeTransforms(parent?.transform, item.declaration?.transform as TransformV2 | undefined);
        const opacity = (parent?.opacity ?? 1) * (typeof item.declaration?.opacity === 'number'
            ? item.declaration.opacity : 1);
