...
export function migrateEditToV2(raw: unknown, options: { hasCaptions?: boolean } = {}): MigrateResult {
    ...
    if (version === 0) {
        ...
        raw.sources.forEach((source, index) => {
            ...
            if (ids.has(source.id)) blockers.push(`edit.json.sources[].id が重複しています: ${source.id}`);
            ids.add(source.id);
            sources.push({
    ...
    narration.forEach((value, index) => {
        ...
        pending.push({ kind: 'audio', ref: audioTrackRefs.narration as number, item });
    });
            ...
            if (!nonEmpty(value.path)
                ...
                pending.push({
                    ...
                    item: {
                        ...
                        ...(value.gain_db !== undefined ? { gain_db: value.gain_db as number } : {}),
                        ...(value.ducking !== undefined ? { ducking: value.ducking as boolean } : {})
                    } as AudioMediaItemV2
                });
            }
        }
...
export function planMigration(
    ...
    const hasCaptions = options.hasCaptions ?? existsSync(captionsPath);
...
function readTrackDefs(
    ...
    if (timeline !== undefined) {
        ...
        const declared = timeline.tracks.flatMap((value, index) => {
            ...
            if (ids.has(value.id)) blockers.push(`timeline.tracks[].id が重複しています: ${value.id}`);
            ids.add(value.id);
            return [{
                id: value.id,
