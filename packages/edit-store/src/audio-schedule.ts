...
import { computeCutTrackSegments, type EditCut } from './edit-store';
...
export function buildWebAudioSchedule(input: WebAudioScheduleInput): WebAudioScheduleResult {
    ...
    const warnUnduckedTarget = (id: string, clipStartSec: number, clipDurationSec: number): void => {
        ...
        if (bgm && isAudioItemAudible(undefined, bgm)) {
            ...
            if (bgm.ducking === true && finitePositive(bgm.durationSec)) {
                ...
                if (clipDurationSec > 0) {
                    warnUnduckedTarget(typeof bgm.id === 'string' && bgm.id ? bgm.id : 'bgm',
            ...
            if (bgm.ducking === undefined && duckKeys.length > 0 && finitePositive(bgm.durationSec)) {
                ...
                const clipDurationSec = finitePositive(bgm.duration)
                    ? Math.min(timelineDurationSec - clipStartSec, bgm.duration) : timelineDurationSec - clipStartSec;
                if (clipDurationSec > 0 && duckIntervals.some(interval =>
                    interval.startSec < clipStartSec + clipDurationSec && interval.endSec > clipStartSec)) {
    ...
    for (const item of sfx) {
        ...
        if (item.spec.ducking === true) {
            warnUnduckedTarget(item.id, item.t,
...
export function projectSpeechDeclarations(
    ...
    const usable = virtualCuts.map((cut, index) => ({ cut, index })).filter(({ cut }) =>
        Number.isFinite(cut.in) && Number.isFinite(cut.out) && cut.in < cut.out);
