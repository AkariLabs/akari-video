// texts[] extrude モード（task 2026-08-12-3d-text-extrude）の検証ハーネス共通ヘルパー。
// 3d-text-flat の evidence（同じ流儀）と対になるが、共有ディレクトリ名を「lib/」にしない —
// リポ直下 .gitignore が `lib/` を一括除外し、許可リストに無い lib/ はコミットされない
// （3d-text-flat/lib/ が現に未コミットのまま埋もれているのを本タスクで発見した。
// governance leak scan 対象ではないので詳細はソースに書かない）。
//
...
export const EVIDENCE_ROOT = resolve(HERE, "..");
export const RENDER_CUT_ROOT = resolve(EVIDENCE_ROOT, "../..");
...
export async function makeProjectRoot(prefix) {
  const projectRoot = await mkdtemp(join(tmpdir(), `3d-text-extrude-${prefix}-`));
...
// opentype の fetch も troika の XHR フォントロードも Chrome のファイル間制限に阻まれる
...
// 使い捨てのローカル静的サーバ（node:http のみ、追加依存なし）
...
// overlay-runtime.js の実プレビュー経路（mount → tick）を、apps/shell を介さず直接ブートするページ
