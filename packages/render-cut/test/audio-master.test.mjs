...
import { chmod, mkdir, mkdtemp, readdir, readFile, rm, symlink, writeFile as rawWriteFile } from "node:fs/promises";
...
function run(project, args = [], env = process.env) {
  ...
  return spawnSync(process.execPath, [cliPath, project, ...legacyRenderArgs(args)], { encoding: "utf8", env });
}
