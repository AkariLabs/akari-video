...
function validateTextStyle(value, label, findings, path) {
  ...
  // 正本は packages/schemas/captions.schema.json の $defs/textStyle、受理条件は
  // render-cut/src/captions.mjs の normalizeTextStyle と 1 対 1。
  const allowed = [
...
const CAPTION_ANIMATION_SLOTS = ["in", "loop", "out"];
const CAPTION_ANIMATION_SLOT_KEYS = ["id", "duration_sec", "ease", "amp"];
...
function validateCaptionAnimation(value, label, findings, path) {
  if (!isRecord(value)) return captionFinding(findings, "captions.text-style", `${label} must be an object`, path);
  for (const field of Object.keys(value)) {
    if (!CAPTION_ANIMATION_SLOTS.includes(field)) {
      captionFinding(findings, "captions.text-style", `${label}.${field} is not a text animation slot`, path);
    }
  }
  for (const slot of CAPTION_ANIMATION_SLOTS) {
    ...
    if (!isRecord(entry)) {
      captionFinding(findings, "captions.text-style", `${slotLabel} must be an object`, path);
      continue;
    }
    for (const field of Object.keys(entry)) {
      if (!CAPTION_ANIMATION_SLOT_KEYS.includes(field)) {
        captionFinding(findings, "captions.text-style", `${slotLabel}.${field} is not defined by the text style contract`, path);
      }
    }
    if (typeof entry.id !== "string" || entry.id === "") {
      captionFinding(findings, "captions.text-style", `${slotLabel}.id must be a non-empty string`, path);
    }
    if (Object.hasOwn(entry, "duration_sec") && (!isFiniteNumber(entry.duration_sec) || entry.duration_sec <= 0)) {
      captionFinding(findings, "captions.text-style", `${slotLabel}.duration_sec must be a positive finite number`, path);
    }
    if (Object.hasOwn(entry, "ease") && (typeof entry.ease !== "string" || entry.ease === "")) {
      captionFinding(findings, "captions.text-style", `${slotLabel}.ease must be a non-empty string`, path);
    }
    if (Object.hasOwn(entry, "amp") && (!isFiniteNumber(entry.amp) || entry.amp <= 0)) {
      captionFinding(findings, "captions.text-style", `${slotLabel}.amp must be a positive finite number`, path);
    }
  }
}
