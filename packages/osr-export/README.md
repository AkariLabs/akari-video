...
起動直後は合成器が空の paint を返す（2026-09-02 実測・契約 §11.8）。Windows で tier 2 の `electron.exe` を起動した直後は `paint` イベントが来ても image が 0×0 か bitmap 長 0 の過渡が Intel iGPU / RTX とも約 0.4〜0.5 秒（12〜15 回）続き、従来の `captureNonEmptyBitmap` は空 paint を **8 回**数えて諦めていたため、`exportWithOsr` の直接呼びは frame 0 で `offscreen paint returned an empty bitmap 8 times` になり本機では直接呼び 17 走 + render-cut 経由 6 走の 23/23 が失敗した（user-data-dir の使い回しは無関係）。`electron-main.mjs` は export / capture の両経路で `settleWindowViewport` の直後・frame 0 の seek の前に **warm-up**（`warmUpOffscreenPaint`）を挟み、非空 bitmap が 1 枚取れるまで paint 要求と `__akariSettle()` を繰り返す（予算 `OSR_WARM_UP_BUDGET_MS` = 5,000 ms・取れた bitmap は捨てる）。結果は run.json（`status: "running"` / 完了 / 失敗とも）の `warm_up: { attempts, empty_attempts, elapsed_ms, satisfied }` に残り、receipt も同じ形の `warm_up` を載せる（`exportWithOsr` が run.json の `warm_up` を `buildOsrReceipt` に渡し、`captureFramesWithOsr` の receipt にも正規化して並べる・無ければ null。render-cut の `.akari/render.json` では `provenance.osr.warm_up`）。予算内に 1 枚も取れなければ `offscreen paint warm-up: <empty_attempts> empty paints over <elapsed_ms> ms（GPU: <active device>）` で fail-closed する。frame ループの `captureNonEmptyBitmap` も回数 8 回固定から **時間予算 2,000 ms（settle の所要込み）と回数 64 回の両方**を上限にし、どちらかに達したら `frame N: offscreen paint returned an empty bitmap <attempts> times over <elapsed_ms> ms（GPU: …）` で失敗する（GPU 名は run.json `gpu.devices` の active の `device_string`・不明なら `unknown`）。run.json の `emptyPaints[]` は `{ frame, attempts, elapsed_ms }`（同じ frame の stamp / hash 再試行は足し込む）。warm-up は seek の前に終わるので frameHashes / 出力には影響しない。本機実測（720p / 30 fps / 10 s・tier 2）: 修正後 Intel iGPU の直接呼び 10 走 = 10/10 完走（`warm_up` empty_attempts 13〜15・461〜510 ms・所要 17.6〜23.5 s = 修正前の成功走行 17〜25 s と同等）、RTX（`AKARI_EXPORT_GPU_PREFERENCE=force`）5 走 = 5/5 完走（12〜14・399〜469 ms・16.9〜19.7 s・実行後レジストリ値なし）、render-cut `--engine osr` 3 走 = 3/3（exit 0・18〜22 s・ffprobe 300 コマ / 10.000 s）、frame ループの `emptyPaints` は全 18 走で空。frameHashes は同じ GPU なら warm-up の回数によらず全 300 コマ一致（Intel 13 走・RTX 5 走）。Intel ↔ RTX は全コマ不一致だが PSNR 平均 48 dB（MSE < 1）の GPU 依存の丸め差で、warm-up とは無関係。
