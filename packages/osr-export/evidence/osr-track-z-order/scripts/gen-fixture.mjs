#!/usr/bin/env node
// osr-track-z-order の L1 fixture（ラッパー作成の検証用。リポの外に出力する）。1280×720・30fps。
...
// e      : ふつうの案件（写真 V2・図形 V3・HTML V4・字幕の袋 V5 = 全部映像の上）8 秒
...
async function finish(dir, tracks, captions) {
    ...
    run('/usr/bin/git', ['config', 'user.email', 'osr-z-fixture@localhost'], dir);
    run('/usr/bin/git', ['config', 'user.name', 'osr z fixture'], dir);
