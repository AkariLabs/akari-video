(() => {
  ...
  window.akari.interaction = (() => {
    ...
    function axisResizeSnap(resize, axis, scaleX, scaleY) {
      ...
      const localDistance = axis === "x" ? cosine * startDx + sine * startDy : -sine * startDx + cosine * startDy;
      const vector = axis === "x" ? { x: cosine * localDistance, y: sine * localDistance } : { x: -sine * localDistance, y: cosine * localDistance };
      const targets = canvasSnapTargets();
      const displayScale = currentDisplayScale();
      let best = null;
        ...
        for (let index = 0; index < targets[coordinate].length; index += 1) {
          ...
          if (distance <= limit && (!best || distance < best.distance)) {
            best = {
              ...
              target,
              distance,
              scale: clampScale((target - anchor) / coefficient * (axis === "x" ? resize.startScaleX : resize.startScaleY))
            };
          }
        }
      }
