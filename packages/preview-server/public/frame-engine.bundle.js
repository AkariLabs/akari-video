  ...
  "../edit-store/lib/caption-words-rederive.js"(exports) {
    ...
    function segmentIntoWords(text) {
      try {
        const Segmenter = segmenterConstructor();
        if (Segmenter) {
          const segmenter = new Segmenter(void 0, { granularity: "word" });
          const tokens = [];
    ...
    function computeDiff(oldKeys, newKeys) {
      ...
      for (let oldIndex2 = 1; oldIndex2 < rows; oldIndex2++) {
        for (let newIndex2 = 1; newIndex2 < columns; newIndex2++) {
          dp[oldIndex2][newIndex2] = oldKeys[oldIndex2 - 1] === newKeys[newIndex2 - 1] ? dp[oldIndex2 - 1][newIndex2 - 1] + 1 : Math.max(dp[oldIndex2 - 1][newIndex2], dp[oldIndex2][newIndex2 - 1]);
        }
      }
      const operations = [];
      ...
      while (oldIndex > 0 || newIndex > 0) {
        if (oldIndex > 0 && newIndex > 0 && oldKeys[oldIndex - 1] === newKeys[newIndex - 1]) {
    ...
    function distributeInInterval(tokens, start, end) {
      ...
      const weights = tokens.map((token) => /^[\p{P}\p{Z}]+$/u.test(token) ? 1e-3 : Math.max(1, toGraphemes(token).length));
...
// ../frame-engine/src/decode/source-selection.ts
...
// ../frame-engine/src/cache/preview-scheduler.ts
