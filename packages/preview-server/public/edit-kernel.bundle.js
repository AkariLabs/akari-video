...
function synthesizeHiddenTransitionHandlesForRender(tracks, fps) {
  ...
  for (const track of tracks) {
    ...
    for (let index = 0; index + 1 < cuts.length; index++) {
      ...
      if (!isRecord(transition) || typeof transition.duration !== "number" || !Number.isFinite(transition.duration) || transition.duration <= 0) continue;
...
function withoutItemAnchors(edit) {
  if (!isRecord2(edit) || !Array.isArray(edit.tracks)) return edit;
  let tracksChanged = false;
  const tracks = edit.tracks.map((track) => {
    if (!isRecord2(track) || !Array.isArray(track.items)) return track;
...
function captionAnchorPositionVars(anchorValue, positionValue, verticalAlignValue) {
  ...
  const position = isRecord3(positionValue) ? positionValue : void 0;
