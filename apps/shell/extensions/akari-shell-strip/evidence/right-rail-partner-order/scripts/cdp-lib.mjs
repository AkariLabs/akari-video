...
// fetch（undici）は負荷の高い機械で setTypeOfService EINVAL を非同期に投げてプロセスごと落ちるため node:http で読む。
export function listTargets(port) {
  return new Promise((resolve, reject) => {
    const request = http.get({ host: '127.0.0.1', port, path: '/json/list', timeout: 5000 }, response => {
      let body = ''; response.setEncoding('utf8'); response.on('data', chunk => { body += chunk; });
      response.on('end', () => { try { resolve(JSON.parse(body)); } catch (error) { reject(error); } });
    });
    request.on('timeout', () => request.destroy(new Error('listTargets timed out')));
