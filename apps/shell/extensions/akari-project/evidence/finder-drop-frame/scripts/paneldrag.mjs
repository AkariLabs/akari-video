...
// (finder-drop-frame: dnd-feedback-polish の同名スクリプトを CDP 9463・WS 必須・edit.json 無しでも動くよう複製)
// HOVER=timeline でパネルの代わりにタイムラインの上へ運ぶ（内部 MIME の委譲先の枠 = 点線ゴーストの記録用）。
// HOVER_POINT=x,y で運ぶ先の点を直接指定する（タイムラインの特定の行など）。
// Usage: WS=<隔離ワークスペース> node paneldrag.mjs <proj:<materialPath>|lib:<catalogKey>> <img|title> <out.json> [drop|cancel] [shot.png]
...
const probe = `(() => { const vis=e=>{const r=e.getBoundingClientRect();return r.width>0&&r.height>0&&getComputedStyle(e).display!=='none'&&getComputedStyle(e).visibility!=='hidden';};
  ...
  return { importOverlayShown: hits.length>0, overlayTexts: hits, timelineGhost: [...document.querySelectorAll('.akari-annotations-widget div')].filter(e => e.style.display === 'block' && /77, 208, 200|241, 76, 76|4dd0c8/.test(e.style.cssText)).map(e => ({ rejected: /241, 76, 76/.test(e.style.cssText), text: e.textContent.trim() })) }; })()`;
if (di) {
  ...
  if (process.env.NO_IMG_FILE !== '1' && ds && ds.types.includes('Files')) {
    ...
    const f = join(process.env.TMPDIR || '.', 'fdf-dragged-thumb') + (src.match(/\.(jpe?g|png|webp)(\?|$)/i)?.[0].replace(/\?$/, '') || '.img');
  ...
  const back = await ev(`(() => { const w=document.querySelector(${JSON.stringify(process.env.HOVER === 'timeline' ? '.akari-annotations-widget' : '#akari-role-buckets-widget')}); const r=w.getBoundingClientRect(); return { x: r.left + r.width/2, y: r.top + r.height*0.6 }; })()`);
  if (process.env.HOVER_POINT) { const [hx, hy] = process.env.HOVER_POINT.split(',').map(Number); back.x = hx; back.y = hy; }
  rec.hoverPoint = back;
