...
const EDIT = `${process.env.WS}/edit.json`; // WS = fixture workspace
...
rec.dockBefore = await ev(`({ dock: !!document.querySelector("[data-akari-catalog-audio-dock]"), audioPaused: window.__lpdAudio ? window.__lpdAudio.paused : null, cardTop: Math.round(document.querySelector("[data-akari-catalog-item]").getBoundingClientRect().top*10)/10 })`);
...
rec.dockAfter = await ev(`({ dock: !!document.querySelector("[data-akari-catalog-audio-dock]"), audioPaused: window.__lpdAudio ? window.__lpdAudio.paused : null, cardTop: Math.round(document.querySelector("[data-akari-catalog-item]").getBoundingClientRect().top*10)/10 })`);
