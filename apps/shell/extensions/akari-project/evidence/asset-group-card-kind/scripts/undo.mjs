...
import { connectMain } from '../../materials-tab-hardening/cdp-lib.mjs';
const [fx, fy, outFile] = process.argv.slice(2);
const EDIT = '/tmp/agck-l1/ws/edit.json';
const cnt = () => { const j = JSON.parse(readFileSync(EDIT, 'utf8')); return { items: j.tracks.flatMap(t => t.items).length, sources: (j.sources||[]).length, tracks: j.tracks.length, sha: JSON.stringify(j).length }; };
