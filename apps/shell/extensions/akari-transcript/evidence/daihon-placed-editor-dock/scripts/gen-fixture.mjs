...
const OUT = path.resolve(process.argv[2] ?? path.join(os.tmpdir(), 'daihon-placed-editor-dock-l1', 'fixture'));
...
async function project(name, withPlaced) {
    ...
    await run('/usr/bin/git', ['config', 'user.email', 'dped-fixture@localhost'], dir);
    await run('/usr/bin/git', ['config', 'user.name', 'dped fixture'], dir);
