...
export async function launch({ shellDir, electron, project, port, isoDir }) {
    ...
    const pid = child.pid;
    let target;
    const deadline = Date.now() + Number(process.env.AKARI_L1_BOOT_MS ?? 180_000);
    ...
    // 負荷の高い機械では起動に数分〜十数分かかる（AKARI_L1_BOOT_MS で上限を延ばす）。失敗時は自分が起こした PID だけを止める。
    const bootMs = Number(process.env.AKARI_L1_BOOT_MS ?? 180_000);
    ...
    try {
        ...
        await waitEval(cdp, `Boolean(window.theia&&window.theia.container&&document.getElementById('theia-app-shell'))`, { label: 'Theia workbench', timeoutMs: bootMs });
