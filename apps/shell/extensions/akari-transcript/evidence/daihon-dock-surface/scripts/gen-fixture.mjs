#!/usr/bin/env node
// 台本の操作面（右クリック = 操作・下端のドック = 編集面）L1 用 fixture を作る。
...
const OUT = path.resolve(process.argv[2] ?? path.join(os.tmpdir(), 'daihon-dock-l1', 'fixture'));
...
async function project(name) {
    ...
    await run('/usr/bin/git', ['config', 'user.email', 'daihon-dock-fixture@localhost'], dir);
    await run('/usr/bin/git', ['config', 'user.name', 'daihon-dock fixture'], dir);
