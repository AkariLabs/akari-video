...
// 台本の行選択 → タイムライン / プレビュー同期を観測するだけなので display_policy は持たせない。
...
await run('/usr/bin/git', ['config', 'user.email', 'daihon-selection-sync-fixture@localhost'], FIXTURE);
await run('/usr/bin/git', ['config', 'user.name', 'Daihon Selection Sync Fixture'], FIXTURE);
await run('/usr/bin/git', ['add', 'captions.json', 'edit.json'], FIXTURE);
await run('/usr/bin/git', ['commit', '-m', '台本選択同期 L1 fixture'], FIXTURE);
