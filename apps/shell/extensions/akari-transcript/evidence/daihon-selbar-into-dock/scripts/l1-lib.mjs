// 台本の置いた文字バー L1 の共通部品（Electron 起動・CDP 接続・待ち・クリック）。
...
// 負荷の高いマシンでは起動に時間がかかる（既定 180 秒・AKARI_L1_LAUNCH_MS で延長）。
const LAUNCH_MS = Number(process.env.AKARI_L1_LAUNCH_MS ?? 180_000);
...
export async function launch({ shellDir, electron, project, port, isoDir, keepProfile = false }) {
    if (!keepProfile) await rm(isoDir, { recursive: true, force: true });
    ...
    try {
        ...
        await waitEval(cdp, `Boolean(window.theia&&window.theia.container&&document.getElementById('theia-app-shell'))`, { label: 'Theia workbench', timeoutMs: LAUNCH_MS });
        await settlePreload(cdp);
    } catch (error) {
        // 起動途中で落ちても自分が起こした Electron だけは必ず止める。
        await stop({ pid, cdp });
        throw error;
    }
    return { pid, cdp };
