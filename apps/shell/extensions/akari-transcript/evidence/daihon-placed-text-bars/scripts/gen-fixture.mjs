...
const OUT = path.resolve(process.argv[2] ?? path.join(os.tmpdir(), 'dptb-l1', 'fixture'));
...
async function project(name, withPlaced) {
    ...
    await run('/usr/bin/git', ['config', 'user.email', 'dptb-fixture@localhost'], dir);
    await run('/usr/bin/git', ['config', 'user.name', 'dptb fixture'], dir);
