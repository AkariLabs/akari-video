...
const groups = wordContextMenuGroups({ rangeCount: 2, wordCount: 4, text: '選択', nextWordText: '次',
  presets: [{ id: 'neon', name: 'Neon' }], splitAvailable: false, mergeAvailable: false,
...
test('declares insertion placeholders and next-row merge action', () => {
  ...
  assert.deepEqual(groups[3].items.find(item => item.action?.kind === 'merge-next')?.action, { kind: 'merge-next' });
});
