...
export async function screenshot(cdp, filePath) {
  const { data } = await cdp.send('Page.captureScreenshot', { format: 'png' }, 240000);
...
/**
 * materials-tab-hardening 追加ヘルパー: 冪等版の developer mode 強制 OFF。
 * この開発機の ~/.theia/settings.json は --user-data-dir で隔離されない共有ファイルで、
 * オーナーの実利用に伴い akari.developerMode が true のまま残っていたり、実行中に
 * 予期せず true へ戻ったりする（原因不明の環境要因を実地で観測）。toggleDeveloperModeViaSettings
 * は無条件トグルで既に OFF のときに ON へ戻してしまうため、チェックボックスの現在値を見てから
 * 必要なときだけクリックする本関数に置き換えた。
 */
export async function ensureDeveloperModeOff(cdp, sleepFn = sleep) {
  const gear = await evalMain(cdp, `(() => {
    const el = Array.from(document.querySelectorAll('.codicon-settings-gear')).find(e => e.getBoundingClientRect().width > 0);
    if (!el) return { found: false };
    const r = el.getBoundingClientRect();
  ...
  const checkbox = await evalMain(cdp, `(() => {
    ...
    if (!cb) return { found: false };
    const r = cb.getBoundingClientRect();
    return { found: true, x: r.left + r.width / 2, y: r.top + r.height / 2, checked: cb.checked };
  })()`);
  if (!checkbox.found) throw new Error('developer mode checkbox not found in AKARI settings panel');
  ...
  if (checkbox.checked) {
    ...
    for (let attempt = 0; attempt < 10; attempt++) {
      await sleepFn(300);
      const stillChecked = await evalMain(cdp, `(() => { const cb = document.querySelector('input[type="checkbox"]'); return cb ? cb.checked : null; })()`);
      if (stillChecked === false) break;
      if (attempt === 9) throw new Error('developer mode checkbox did not turn off');
    }
  }
  ...
  if (!filesIcon.found) throw new Error('files/explorer activity icon not found after ensuring developer mode off');
  await realClick(cdp, filesIcon.x, filesIcon.y);
  await sleepFn(500);
  return { wasChecked: checkbox.checked, toggled };
}
