...
export async function launch({ shellDir, electron, project, port, isoDir, environment }) {
    ...
    const childEnv = { PATH: process.env.PATH, TMPDIR: process.env.TMPDIR, LANG: process.env.LANG,
        SHELL: process.env.SHELL, USER: process.env.USER, ...environment };
    ...
    try {
        ...
        if (!commandLine.includes(electron) || !commandLine.includes(`--remote-debugging-port=${port}`)) {
            throw new Error('CDP port process does not match this Electron launch');
        }
        let target;
