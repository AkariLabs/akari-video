...
export async function run(argv = process.argv.slice(2)) {
  ...
  const metaPath = path.join(root, `${source.path}.meta.json`);
  const base = {
    version: 1, kind: 'video', inputs: {}, output: { duration_s: 6 },
  ...
  await wait(3000);
  await fs.writeFile(metaPath, `${JSON.stringify({ ...base, status: 'done' }, null, 2)}\n`);
