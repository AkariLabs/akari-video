...
import {
    canvasChildChipSpan, cutMediaRowHeight, timelineCanvasRowGeometry, timelineItemRenderRoute,
    timelineTrackHeight, TimelineCanvasRowGeometry
} from './timeline/timeline-canvas-row-layout';
import {
...
export class AkariAnnotationsWidget extends BaseWidget {
    ...
    protected treeRowGeometry = new Map<string, TimelineCanvasRowGeometry>();
    ...
    /** 字幕以外の per-track 高さ。キー = EditTimelineTrack.id。StorageService から遅延読み込み。 */
...
`;
            ...
            switch (request.kind) {
                ...
                case 'caption-style-my-style': {
                    ...
                    const editBefore = attachParts.length
                        ? (await this.fileService.readFile(location.editUri)).value.toString() : undefined;
                    const editAfter = editBefore && typeof request.value.style_uid === 'string'
                        ? `${JSON.stringify(resolveItemAnchors(applyMyStyleAttachedParts(
                            JSON.parse(editBefore), toAnchorCaptions(JSON.parse(after)), ids,
                            request.value.style_uid, attachParts) as unknown as EditV2, toAnchorCaptions(JSON.parse(after))).edit, null, 2)}\n`
                        : undefined;
                    const write = async (captionsSource: string, editSource?: string): Promise<void> => {
                        await this.annotationsService.writeEditSnapshot({
                            editUri, projectRootUri: location.root.toString(),
                            captionsUri: captionsUri.toString(), captionsSource,
    ...
    protected async openMyStyleSaveDialog(captionId?: string): Promise<void> {
        ...
        const attachedParts = this.location?.editUri
            ...
            : [];
        const available = (kind: string): boolean => attachedParts.some(part => part.kind === kind);
        ...
        const save = async (): Promise<void> => {
            ...
            try {
                ...
                this.showNotice(error instanceof Error ? error.message : String(error)); return;
            }
            const now = new Date().toISOString();
            const style = { schema: 'akari-style', version: 1, revision: 1, uid: newMyStyleUid(),
                id: newMyStyleSlug(name.value.trim()), name: name.value.trim(),
                ...
                parts: [...myStyleSaveParts(this.defaultTextStyle, caption.textStyle, height,
                    [lookChecked ? 'look' : '', motionChecked ? 'motion' : '']), ...attachedSelected] };
    ...
    async placeText(options: PlaceTextOptions & { myStyle?: { uid?: string; revision?: number;
        ...
        try {
            ...
            const parsedCaptions = parseCaptions(source);
            const captions = parsedCaptions.captions;
            ...
            const attachedParts = options.myStyle?.parts.filter(supportedMyStyleAttachPart) ?? [];
            ...
            await this.withHistory('文字を置く', async () => {
                if (canvas) {
                    ...
                    const editAfter = resolveItemAnchors(next as unknown as EditV2, toAnchorCaptions(JSON.parse(captionsSource))).edit;
                    await this.annotationsService.writeEditSnapshot({
                        editUri: location.editUri!.toString(), projectRootUri: location.root.toString(),
                        captionsUri: location.captionsUri.toString(), captionsSource,
                        editSource: `${JSON.stringify(editAfter, null, 2)}\n`
            ...
            if (options.myStyle && message.includes('layout')) this.showNotice(message);
            return undefined;
        } finally {
        ...
        try {
            ...
            if (captionsAfter !== undefined) {
                for (const id of idsByKind.get('caption') ?? []) {
                    ...
                    value = removeStyleAttachedItems(value as EditV2, id) as Record<string, any>;
                }
            }
    ...
    protected visualTrack(track: EditTimelineTrack): boolean {
        const leaves = this.expandedTimelineTreeRows.filter(row => row.trackId === track.id && row.sourceKind !== 'group');
        if (leaves.length && leaves.every(row => row.sourceKind === 'caption' || row.sourceKind === 'captions')) return false;
        return track.kind === 'layers' || track.kind === 'overlays' || leaves.some(row => row.sourceKind === 'html');
    }

    protected timelineRowStride(trackId: string): number {
        const track = this.displayTimelineTracks.find(entry => entry.id === trackId);
    ...
    protected calculateLaneLayout(topOffset = 0): number {
        ...
        for (const timelineTrack of [...this.displayTimelineTracks].reverse()) {
            ...
            const rowStride = this.timelineRowStride(timelineTrack.id);
            ...
            if (timelineTrack.kind === 'cuts') {
                ...
                const chipLayout = assignDetachedCaptionChipSubRows(
                    ...
                    captionRows,
                    { placement: 'mix', baseHeight: rowStride, subrowStride: rowStride }
                );
            ...
            const hasOrdinaryItem = this.timelineTreeTracks.find(track => track.id === timelineTrack.id)
                ?.items.some(item => item.source.kind !== 'group') ?? false;
            const treeGeometry = timelineCanvasRowGeometry(
                treeRows, rowStride, SUBROW_STRIDE, SUBROW_GAP,
                id => this.keyframeRowsByItem.get(id)?.length ?? 0,
                hasOrdinaryItem ? rowStride : SUBROW_STRIDE
            );
            for (const [id, geometry] of treeGeometry.rows) this.treeRowGeometry.set(id, geometry);
            height = timelineTrackHeight(height, treeGeometry);
            if (!captionRowsMixedWithItems && captionRows.length > 0) {
                const chipLayout = assignDetachedCaptionChipSubRows([], captionRows, {
                    placement: 'append', baseHeight: height, subrowStride: rowStride
                });
                captionRows.forEach(row => this.overlayRows.set(row.id, chipLayout.rowById.get(row.id) ?? 0));
                height = chipLayout.height;
            }
    ...
    protected renderStrip(): void {
        ...
        ].filter(id => timelineItemRenderRoute(id, this.itemLocations, this.timelineTreeRows) === 'legacy'));
        ...
        for (const row of [
            ...
            const { element, created } = this.keyedStripSegment(
                ...
                layout.top + (this.treeRowGeometry.get(row.id)?.top ?? rowIndex * this.timelineRowStride(row.trackId)),
                this.treeRowGeometry.get(row.id)?.height ?? this.timelineRowStride(row.trackId) - SUBROW_GAP,
            ...
            element.setAttribute('data-akari-ui', row.sourceKind === 'group' ? 'timeline-canvas-chip' : 'timeline-tree-chip');
            ...
            if (created) {
                ...
                if (row.sourceKind === 'group' && row.collapsed) {
                    for (const child of this.expandedTimelineTreeRows.filter(candidate => candidate.parentId === row.id)) {
                        const span = canvasChildChipSpan(row, child);
                        if (!span) continue;
                        const childChip = document.createElement('span');
                        childChip.dataset.akariCanvasChild = child.id;
                        childChip.title = child.label;
                        Object.assign(childChip.style, {
                            position: 'absolute', top: '0', height: `${SUBROW_GAP}px`,
                            left: `${span.left * 100}%`, width: `${span.width * 100}%`,
                            background: 'var(--theia-focusBorder)', pointerEvents: 'none'
                        });
                        element.appendChild(childChip);
                    }
                }
        ...
        this.overlays.forEach(overlay => {
            if (timelineItemRenderRoute(overlay.id, this.itemLocations, this.timelineTreeRows) !== 'legacy') return;
            ...
            const { element, created } = this.keyedStripSegment(
                `overlay:${overlay.id}`, JSON.stringify(overlay), overlay.start, end, top, stride - SUBROW_GAP,
        ...
        this.layers.forEach(layer => {
            if (timelineItemRenderRoute(layer.id, this.itemLocations, this.timelineTreeRows) !== 'legacy') return;
        ...
        this.segments.forEach(segment => {
            ...
            if (timelineItemRenderRoute(cutItemId, this.itemLocations, this.timelineTreeRows) !== 'legacy') return;
            ...
            const cutTrack = this.displayTimelineTracks.find(track => track.id === cutLayout.id);
            const cutRowHeight = cutMediaRowHeight(
                cutLayout.height,
                cutTrack ? this.trackHeightFor(cutTrack) : undefined,
                !!this.treeRowsByTrack.get(cutLayout.id)?.length
            );
            const cut = this.cuts[segment.index];
            ...
            const cutSignature = JSON.stringify([
                cut, segment, this.videoClipLabel(cutItemId, cut), cutRowHeight, cutTrimmerActive,
            ...
            const { element, created } = this.keyedStripSegment(
                ...
                cutLayout.top, cutRowHeight, 'akari-annotations-strip-clip', `C${segment.index + 1}`
            );
            ...
            if (showTrimmer) {
                if (created) {
                    this.renderTrimmerClip(
                        element, cut, clipWidth, segment, cutRowHeight, trimmerVideoUri, trimmerSourceDuration
                    );
                ...
                if (created) {
                    this.renderClipMedia(element, cut, clipWidth, segment, cutRowHeight);
                    element.appendChild(this.clipHeader(this.videoClipLabel(cutItemId, cut), segment.tlEnd - segment.tlStart));
                    this.appendClipKindBadge(element, this.rawKeyframeItem(cutItemId) ?? { source: { kind: 'media' } }, { path: cut.src, lane: 'visual' });
                } else {
                    this.updateClipMediaGeometry(element, cut, clipWidth, segment, cutRowHeight);
                }
                this.installDragListeners(element, (event, rect) => {
                    const edgeMode = this.resolveClipEdgeMode(event, rect, element);
                    if (edgeMode === 'start') {
    ...
    protected renderKeyframePropertyRows(row: TimelineTreeRow, trackTop: number, itemRowIndex: number): void {
        const stride = this.timelineRowStride(row.trackId);
        const raw = this.rawKeyframeItem(row.id);
        ...
        rows.forEach((propertyRow, index) => {
            ...
            const { element, created } = this.keyedStripSegment(
                ...
                trackTop + (this.treeRowGeometry.get(row.id)?.propertyTops[index]
                    ?? (itemRowIndex + index + 1) * stride),
                stride - SUBROW_GAP, 'akari-timeline-keyframe-property-row', propertyRow.label
            );
            element.style.pointerEvents = 'auto';
    ...
    protected decorateTreeTrackHeader(header: HTMLDivElement, rows: readonly TimelineTreeRow[]): void {
        ...
        rows.forEach(treeRow => {
            ...
            const geometry = this.treeRowGeometry.get(treeRow.id);
            const row = document.createElement('div');
            ...
            Object.assign(row.style, {
                position: 'absolute', left: '0', right: '0', top: `${geometry?.top ?? stride}px`,
                height: `${geometry?.height ?? stride - SUBROW_GAP}px`,
            ...
            for (const [index, propertyRow] of (this.keyframeRowsByItem.get(treeRow.id) ?? []).entries()) {
                ...
                Object.assign(property.style, {
                    position: 'absolute', left: '0', right: '0', top: `${geometry?.propertyTops[index] ?? stride}px`,
                    height: `${stride - SUBROW_GAP}px`, paddingLeft: `${24 + treeRow.depth * 16}px`,
                    boxSizing: 'border-box', fontSize: '10px', lineHeight: `${stride - SUBROW_GAP}px`,
    ...
    protected async performLinkedDeletion(
        ...
        try {
            ...
            await this.commitEditMutation(label, doc => {
                ...
                for (const caption of captions) next = removeStyleAttachedItems(next as unknown as EditV2, caption.id) as unknown as EditV2Document;
                return next;
    ...
    protected async performEditMutation(
        ...
        const resolved = options?.captions
            ? resolveItemAnchors(distribution.document as unknown as EditV2,
                toAnchorCaptions(JSON.parse(options.captions.after))).edit as unknown as EditV2Document
            : distribution.document;
        const after = stringifyEditV2(resolved);
    ...
    protected async commitEditV2Drag(preview: DragPreview): Promise<void> {
        ...
        try {
            ...
            const movedId = preview.kind === 'cut-move' ? this.cutItemId(preview.index)
                : preview.kind === 'layer' || preview.kind === 'audio' || preview.kind === 'overlay-move'
                    ? preview.id : undefined;
            const mutation = mutate;
            await this.commitEditMutation(label, doc => {
                const next = mutation(doc);
                if (movedId && this.rawV2Item(movedId)?.anchor?.attached_by) {
                    detachMovedStyleItem(next as unknown as Parameters<typeof detachMovedStyleItem>[0], movedId);
                }
                return next;
            }, { optimistic: true });
            if (createdTrackId) {
    ...
    protected async openTimelineClipContextMenu(event: MouseEvent, element: HTMLElement): Promise<void> {
        ...
        const baseItems = buildTimelineClipMenuItems(
            item.kind === 'item' ? (item.itemKind === 'caption' ? 'caption' : 'overlay') : item.kind,
    ...
    protected async openCaptionAttachDialog(itemId: string, x: number, y: number): Promise<void> {
        const raw = this.rawV2Item(itemId);
        if (!raw) return;
        const at = raw.at / this.fps;
        const candidates = this.captions.filter(caption => {
            const layout = this.captionLayouts.get(caption.id);
        ...
        Object.assign(popup.style, { position: 'fixed', zIndex: '100000', left: `${Math.min(x, window.innerWidth - 260)}px`,
        ...
        for (const caption of candidates) {
            ...
            option.textContent = `${caption.id} ${caption.text.slice(0, 24)}`; captionSelect.appendChild(option);
        }
        ...
        button.addEventListener('click', () => {
            ...
            void this.commitEditMutation('字幕にひも付ける', doc => {
                ...
                for (const track of next.tracks as Array<{ items?: Array<Record<string, any>> }>) {
                    ...
                    if (!target) continue;
                    target.anchor = { caption: captionId, edge, duration };
                }
                return resolveItemAnchors(next as unknown as EditV2, toAnchorCaptions(this.captions)).edit as unknown as EditV2Document;
    ...
    protected dispatchTimelineClipMenuAction(
        ...
        if (id === 'narrate' && (item.kind === 'caption' || (item.kind === 'item' && item.itemKind === 'caption'))) {
            const captionId = item.kind === 'caption' ? item.id
                : this.captions.find(caption => item.id.endsWith(`#${caption.id}`))?.id
                    ?? (this.rawV2Item(item.id)?.source?.kind === 'caption'
                        ? this.rawV2Item(item.id)?.source?.id : undefined);
            if (!captionId) return;
            const selected = this.selectionModel.selectedCaptionIds;
            void this.openReadAloud({ captionIds: selected.length >= 2 && selected.includes(captionId) ? selected : [captionId] });
            return;
        }
