...
export type RightPaneSyncAction = 'attach-inspector' | 'skip';

export function resolveRightPaneSyncAction(
    currentRightWidgetId: string | undefined,
    hasSelection: boolean
): RightPaneSyncAction {
    // Selection may create the tab, but switching the visible panel is always manual.
    return hasSelection && currentRightWidgetId !== AKARI_INSPECTOR_WIDGET_ID ? 'attach-inspector' : 'skip';
}
