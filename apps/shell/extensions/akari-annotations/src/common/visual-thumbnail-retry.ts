...
export function visualThumbnailRetryPlan(failure: unknown, attempt: number, now: number): VisualThumbnailRetryPlan {
    ...
    if (/no visible pixels|Invalid visual thumbnail page|Visual renderer failed|Visual renderer readiness timed out|edit snapshot mismatch|no renderable overlay|Thumbnail input is outside|thumbnail stylesheet|ENOENT|SyntaxError|TypeError/i.test(message)) {
