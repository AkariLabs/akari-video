...
export async function launch({ shellDir, electron, project, port, isoDir }) {
    ...
    child.unref();
    const pid = child.pid;
    if (!Number.isInteger(pid) || pid <= 0) throw new Error('Electron PID を取得できません');
    await writeFile(path.join(isoDir, 'electron.pid'), `${pid}\n`);
    let cdp;
    try {
        let target;
...
export async function stop(session) {
    session?.cdp?.close();
    if (session?.pid) {
        // Electron Helper は別 PGID に入る場合がある。起動した PID の子孫だけを辿る。
        const rows = spawnSync('ps', ['-axo', 'pid=,ppid='], { encoding: 'utf8' }).stdout
            ?.trim().split('\n').map(line => line.trim().split(/\s+/u).map(Number)) ?? [];
        const owned = [session.pid];
