...
// timeline selection/copy-paste acceptance criteria plus Wave 21 regressions against a running
...
// 一度でもプレビュー webview（別レンダラープロセスの guest view）を開閉・操作すると、CDP の
// Input.dispatchKeyEvent がメインウィンドウの window ではなく webview 側に配送され続ける現象を
// 実測（overlay-runtime 側の独自 Escape ハンドラが誤って反応する）。キーボードショートカット系の
// 検証（Escape/Cmd+C/Cmd+V/Cmd+Z 等）の直前に、タイムラインウィジェットの安全な非コンテンツ領域
// （ツールバー）を実クリックしてメインレンダラーへ確実にフォーカスを戻す。
...
async function main() {
  ...
  // ================= selection + caption copy/paste =================
  const captionsBeforeNoopPaste = await readFile(CAPTIONS_JSON_PATH, 'utf8');
  const editBeforeNoopPaste = await readFile(EDIT_JSON_PATH, 'utf8');
  await keyPress(main, { key: 'v', code: 'KeyV', windowsVirtualKeyCode: 86, modifiers: 4 });
  await sleep(250);
  assert(await readFile(CAPTIONS_JSON_PATH, 'utf8') === captionsBeforeNoopPaste
    && await readFile(EDIT_JSON_PATH, 'utf8') === editBeforeNoopPaste,
  'paste without an application clipboard is harmless');

  const captionRectForSelection = await elementRect(main, '.akari-annotations-strip-caption', 0);
  await realClick(main,
    (captionRectForSelection.left + captionRectForSelection.right) / 2,
    captionRectForSelection.top + captionRectForSelection.height / 2);
  await sleep(250);
  let selectedItems = await selectedTimelineItems(main);
  assert(selectedItems.length === 1 && selectedItems[0].kind === 'caption'
    && selectedItems[0].id === 'caption-a' && selectedItems[0].className.includes('akari-annotations-selected'),
  'caption pointerup selects exactly one timeline item', { selectedItems });
  await shot(main, '20-caption-selected.png');

  await keyPress(main, { key: 'Escape', code: 'Escape', windowsVirtualKeyCode: 27 });
  await sleep(150);
  assert((await selectedTimelineItems(main)).length === 0, 'Escape clears timeline selection');
  ...
  assert(selectedItems.length === 1 && selectedItems[0].id === 'caption-a',
    'caption selection survives renderStrip after an external file update', { selectedItems });

  await keyPress(main, { key: 'c', code: 'KeyC', windowsVirtualKeyCode: 67, modifiers: 4 });
  await sleep(100);
  const beforeInputGuard = await readFile(CAPTIONS_JSON_PATH, 'utf8');
  await evalOn(main, `(() => {
    const input = document.createElement('input');
    input.id = 'timeline-copy-paste-input-guard';
    document.body.appendChild(input);
    input.focus();
  })()`);
  await keyPress(main, { key: 'v', code: 'KeyV', windowsVirtualKeyCode: 86, modifiers: 4 });
  await sleep(250);
  assert(await readFile(CAPTIONS_JSON_PATH, 'utf8') === beforeInputGuard,
    'Cmd+V is harmless while an input is focused');
  await evalOn(main, `document.getElementById('timeline-copy-paste-input-guard')?.remove()`);

  const captionPasteTime = 7;
  const captionPasteX = await screenXForTime(main, captionPasteTime);
  const captionPasteState = await widgetState(main);
  await realClick(main, captionPasteX, captionPasteState.stripRect.top + 7);
  await sleep(200);
  const footerBeforeCaptionPaste = await readFooter(main);
  await keyPress(main, { key: 'v', code: 'KeyV', windowsVirtualKeyCode: 86, modifiers: 4 });
  await waitFooterSettled(main, /字幕をペーストしました/, footerBeforeCaptionPaste);
  const captionsAfterPasteSource = await readFile(CAPTIONS_JSON_PATH, 'utf8');
  const captionsAfterPaste = JSON.parse(captionsAfterPasteSource);
  const pastedCaption = captionsAfterPaste.find(caption => caption.id !== 'caption-a' && caption.id !== 'caption-b');
  // クリックで立てた playheadT はストリップの px 量子化（実測 ~0.013秒/px）を経由するため、
  // 意図したクリック目標時刻ちょうどには一致しない。ペースト先は実際の playheadT なので、
  // 妥当な許容誤差（1px 相当を大きく超える 0.05秒）で比較する。
  assert(Boolean(pastedCaption) && Math.abs(pastedCaption.start - captionPasteTime) < 0.05
    && Math.abs((pastedCaption.end - pastedCaption.start) - 1.5) < 1e-6 && pastedCaption.text === 'first caption',
  'caption paste preserves duration/text and assigns a new id at the playhead (within click px-quantization tolerance)',
  { pastedCaption, captionPasteTime });
  assert(captionsAfterPasteSource.split(/\r?\n/).some(line => line.includes(`"id": "${pastedCaption.id}"`)
    && line.includes('{') && line.includes('}')),
  'pasted caption remains one caption per line');
  await shot(main, '21-caption-pasted.png');

  let footerBeforeHistory = await readFooter(main);
  await keyPress(main, { key: 'z', code: 'KeyZ', windowsVirtualKeyCode: 90, modifiers: 4 });
  await waitFooterSettled(main, /字幕のペーストを元に戻しました/, footerBeforeHistory);
  assert(!(await readFile(CAPTIONS_JSON_PATH, 'utf8')).includes(pastedCaption.id),
    'caption paste undo removes the inserted id');
  footerBeforeHistory = await readFooter(main);
  await keyPress(main, { key: 'z', code: 'KeyZ', windowsVirtualKeyCode: 90, modifiers: 12 });
  await waitFooterSettled(main, /字幕のペーストをやり直しました/, footerBeforeHistory);
  assert((await readFile(CAPTIONS_JSON_PATH, 'utf8')).includes(pastedCaption.id),
    'caption paste redo restores the same inserted id');
  ...
  // widget 側の多数の write 操作（drag/undo/redo）の間に webview がリロードされ、古い
  // executionContextId が無効化されることを実測（'Cannot find context with specified id'）。
  // 再接続できるよう outer/activeCtx を let にし、connectPreview() を後段でも呼び直せるようにする。
  let outer;
  let activeCtx;
  ...
  // ================= timeline <-> preview overlay selection + overlay copy/paste =================
  const overlayBeforeSelection = (await readJson(EDIT_JSON_PATH)).overlays[0];
  await scrollToTime(main, overlayBeforeSelection.start + overlayBeforeSelection.duration / 2);
  await sleep(200);
  const overlayRectForSelection = await elementRect(main, '.akari-annotations-strip-overlay', 0);
  await realClick(main,
    (overlayRectForSelection.left + overlayRectForSelection.right) / 2,
    overlayRectForSelection.top + overlayRectForSelection.height / 2);
  await sleep(900);
  const previewSelectionFromTimeline = await evalInPreview(`(() => {
    const selected = document.querySelector('[data-overlay-id][data-akari-interaction-selected="true"]');
    return selected ? { id: selected.getAttribute('data-overlay-id'), selected: selected.getAttribute('data-akari-interaction-selected') } : null;
  })()`);
  selectedItems = await selectedTimelineItems(main);
  assert(previewSelectionFromTimeline?.id === overlayBeforeSelection.id
    && previewSelectionFromTimeline.selected === 'true'
    && selectedItems.length === 1 && selectedItems[0].id === overlayBeforeSelection.id,
  'timeline overlay selection uses interaction click behavior in the preview webview', {
    previewSelectionFromTimeline, selectedItems
  });
  await shot(main, '22-timeline-to-preview-overlay-selected.png');

  await focusWidgetToolbar(main);
  await keyPress(main, { key: 'Escape', code: 'Escape', windowsVirtualKeyCode: 27 });
  await sleep(500);
  await evalInPreview(`(() => {
    const target = document.querySelector('[data-overlay-id="${overlayBeforeSelection.id}"]');
    const fragment = Array.from(target.children).find(candidate => !candidate.hasAttribute('data-akari-interaction')) || target;
    const rect = fragment.getBoundingClientRect();
    target.dispatchEvent(new MouseEvent('click', {
      bubbles: true, cancelable: true, composed: true,
      clientX: rect.left + rect.width / 2, clientY: rect.top + rect.height / 2
    }));
    return target.getAttribute('data-akari-interaction-selected');
  })()`);
  await sleep(500);
  selectedItems = await selectedTimelineItems(main);
  assert(selectedItems.length === 1 && selectedItems[0].kind === 'overlay'
    && selectedItems[0].id === overlayBeforeSelection.id,
  'preview overlay click highlights the matching timeline item without seeking', { selectedItems });
  await shot(main, '23-preview-to-timeline-overlay-selected.png');

  await focusWidgetToolbar(main);
  await keyPress(main, { key: 'c', code: 'KeyC', windowsVirtualKeyCode: 67, modifiers: 4 });
  const overlayPasteTime = 8;
  await scrollToTime(main, overlayPasteTime);
  await sleep(150);
  const overlayPasteX = await screenXForTime(main, overlayPasteTime);
  const overlayPasteState = await widgetState(main);
  await realClick(main, overlayPasteX, overlayPasteState.stripRect.top + 7);
  await sleep(200);
  const editRawBeforeOverlayPaste = await readFile(EDIT_JSON_PATH, 'utf8');
  const overlaysCloseBefore = editRawBeforeOverlayPaste.lastIndexOf('\n  ]');
  const originalOverlayEnd = editRawBeforeOverlayPaste.lastIndexOf('\n    }', overlaysCloseBefore) + '\n    }'.length;
  const footerBeforeOverlayPaste = await readFooter(main);
  await focusWidgetToolbar(main);
  await keyPress(main, { key: 'v', code: 'KeyV', windowsVirtualKeyCode: 86, modifiers: 4 });
  await waitFooterSettled(main, /オーバーレイをペーストしました/, footerBeforeOverlayPaste);
  const editRawAfterOverlayPaste = await readFile(EDIT_JSON_PATH, 'utf8');
  const editAfterOverlayPaste = JSON.parse(editRawAfterOverlayPaste);
  const pastedOverlay = editAfterOverlayPaste.overlays.find(overlay => overlay.id !== overlayBeforeSelection.id);
  const outsidePrefixPreserved = editRawAfterOverlayPaste.startsWith(editRawBeforeOverlayPaste.slice(0, originalOverlayEnd));
  const outsideSuffixPreserved = editRawAfterOverlayPaste.endsWith(editRawBeforeOverlayPaste.slice(overlaysCloseBefore));
  // caption paste と同様、クリックで立てた playheadT は px 量子化を経由するため 0.05秒の許容誤差で比較する。
  assert(Boolean(pastedOverlay) && pastedOverlay.id.startsWith(`${overlayBeforeSelection.id}-copy`)
    && Math.abs(pastedOverlay.start - overlayPasteTime) < 0.05 && pastedOverlay.duration === overlayBeforeSelection.duration
    && JSON.stringify(pastedOverlay.transform) === JSON.stringify(overlayBeforeSelection.transform)
    && JSON.stringify(pastedOverlay.vars) === JSON.stringify(overlayBeforeSelection.vars),
  'overlay paste preserves the full JSON payload and assigns a collision-free copy id (within click px-quantization tolerance)',
  { pastedOverlay, overlayPasteTime });
  assert(outsidePrefixPreserved && outsideSuffixPreserved,
    'overlay insertion preserves existing overlay bytes and all text outside overlays[]', {
      outsidePrefixPreserved, outsideSuffixPreserved
    });
  await shot(main, '24-overlay-pasted.png');

  footerBeforeHistory = await readFooter(main);
  await focusWidgetToolbar(main);
  await keyPress(main, { key: 'z', code: 'KeyZ', windowsVirtualKeyCode: 90, modifiers: 4 });
  await waitFooterSettled(main, /オーバーレイのペーストを元に戻しました/, footerBeforeHistory);
  assert(await readFile(EDIT_JSON_PATH, 'utf8') === editRawBeforeOverlayPaste,
    'overlay paste undo restores edit.json byte-for-byte');
  footerBeforeHistory = await readFooter(main);
  await focusWidgetToolbar(main);
  await keyPress(main, { key: 'z', code: 'KeyZ', windowsVirtualKeyCode: 90, modifiers: 12 });
  await waitFooterSettled(main, /オーバーレイのペーストをやり直しました/, footerBeforeHistory);
  const editAfterOverlayRedo = await readJson(EDIT_JSON_PATH);
  assert(editAfterOverlayRedo.overlays.some(overlay => overlay.id === pastedOverlay.id),
    'overlay paste redo restores the same inserted id');
  ...
  // AC9/AC7/AC8 の一連の write 操作の間に webview がリロードされ、activeCtx が失効することを
  // 実測したため、以降 evalInPreview を使う前に再接続する。
  await connectPreview();
  ...
  // Chromium のメディアクロックが進行せず currentTime が完全に静止することを実測した
  // （muted=false: 1.5秒待っても currentTime 不動、muted=true: 同条件で正常に進行）。
  // tick()/playheadT 同期ロジックは video.currentTime/paused のみを見て muted を一切参照しない
  // ため、ミュートしても検証対象の同期挙動そのものは変わらない。環境の音声出力欠如を
  // 回避するための検証ドライバ側の対処として明示的に mute する。
