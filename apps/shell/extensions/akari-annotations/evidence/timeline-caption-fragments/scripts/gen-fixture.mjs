...
// 台本の分割 / 結合 / ／（改行）→ タイムライン字幕帯の即時反映を観測するだけなので display_policy は持たせない
// （手で置いた display_fragments だけを目盛りの入力にする）。
...
await run('/usr/bin/git', ['config', 'user.email', 'timeline-caption-fragments-fixture@localhost'], FIXTURE);
await run('/usr/bin/git', ['config', 'user.name', 'Timeline Caption Fragments Fixture'], FIXTURE);
await run('/usr/bin/git', ['add', 'captions.json', 'edit.json'], FIXTURE);
await run('/usr/bin/git', ['commit', '-m', 'タイムライン字幕断片 L1 fixture'], FIXTURE);
process.stdout.write(`${JSON.stringify({
  ok: true,
  rows: rows.length,
  mediaSeconds,
  captions: rows.map(row => ({ id: row.id, text: row.text, start: row.start, end: row.end, words: row.words.length }))
})}\n`);
