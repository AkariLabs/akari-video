#!/bin/bash
# 起動〜ライブラリの画像カテゴリを開いてプレイヘッドを置くまで。使い方: bash setup.sh <repo> <work> <ws名> <seek秒>
set -uo pipefail
REPO=$1; W=$2; WS=$3; SEEK=$4; HERE=$(cd "$(dirname "$0")" && pwd)
export CDP_PORT=${CDP_PORT:-9622} AKARI_CDP_TIMEOUT_MS=${AKARI_CDP_TIMEOUT_MS:-60000}
...
cd "$HERE"
bash "$HERE/post-launch.sh" "$W/$WS" "$SEEK"; exit_code=$?
