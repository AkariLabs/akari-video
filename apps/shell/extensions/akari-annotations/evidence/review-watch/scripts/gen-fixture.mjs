...
const PROJECT = path.join(ROOT, 'runs', 'l1', 'project');
...
await rm(path.join(ROOT, 'runs', 'l1'), { recursive: true, force: true });
...
process.stdout.write(`${JSON.stringify({ ok: true, clips: edit.tracks?.[0]?.items?.map(item => item.id) ?? [] })}\n`);
