...
// 使い方: node pvdrag.mjs <project> <card> <drop> <out.json> [--shot=<png>] [--cancel]
...
rec.drop = drop;
const items = t => JSON.parse(t).tracks.flatMap((tr, trackIndex) => tr.items.map(i => ({ track: tr.id, trackIndex, trackName: tr.name, lane: tr.lane, id: i.id, at: i.at, duration: i.duration, source: i.source, transform: i.transform })));
...
const PROBE = `(()=>{const vis=e=>{const r=e.getBoundingClientRect();return r.width>0&&r.height>0&&getComputedStyle(e).display!=='none'&&getComputedStyle(e).visibility!=='hidden'};
 ...
 const texts=[...document.querySelectorAll('*')].filter(e=>e.children.length===0&&vis(e)&&/→/.test(e.textContent)&&e.textContent.length<40).map(e=>({text:e.textContent.trim(),rect:rect(e)}));
 ...
 return{layer:layer.map(e=>({tag:e.tagName,cls:String(e.className).slice(0,80),data:[...e.attributes].filter(a=>a.name.startsWith('data-')).map(a=>a.name+'='+a.value.slice(0,40)),rect:rect(e),pointerEvents:getComputedStyle(e).pointerEvents})),
  ...
  times:texts,svgs,
  applyTargets:[...document.querySelectorAll('[data-akari-library-apply-target]')].filter(vis).map(e=>({id:e.dataset.akariItemId??null,kind:e.dataset.akariItemKind??null,value:e.getAttribute('data-akari-library-apply-target'),rect:rect(e),outline:getComputedStyle(e).outlineStyle+' '+getComputedStyle(e).outlineColor})),
