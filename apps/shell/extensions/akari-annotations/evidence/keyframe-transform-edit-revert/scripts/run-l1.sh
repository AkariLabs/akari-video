...
PORT=${AKARI_CDP_PORT:-9493}
WORKSPACE=$(cd "$(mktemp -d -t keyframe-transform-edit-revert-workspace)" && pwd -P)
USERDATA=$(cd "$(mktemp -d -t keyframe-transform-edit-revert-userdata)" && pwd -P)
CONFIGDIR=$(cd "$(mktemp -d -t keyframe-transform-edit-revert-config)" && pwd -P)
AKARIHOME=$(cd "$(mktemp -d -t keyframe-transform-edit-revert-home)" && pwd -P)
