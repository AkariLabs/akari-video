...
if (args[1] === 'engines') {
  ...
  const seconds = Number(control.seconds);
  if (!Number.isFinite(seconds) || seconds <= 0 || seconds > 30) throw new Error('unexpected L1 duration');
  if (control.mode === 'slow') {
    await new Promise(resolve => { const timer = setTimeout(resolve, Number(control.delaySeconds ?? 4) * 1000);
      process.once('SIGTERM', () => { clearTimeout(timer); process.exit(143); }); });
  }
  if (control.mode === 'failure') { process.stderr.write('L1 の意図した失敗です\n'); process.exit(2); }
