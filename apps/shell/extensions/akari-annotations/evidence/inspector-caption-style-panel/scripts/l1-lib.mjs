// L1 の共通部品（akari-preview/evidence/placed-text-position-polish/scripts/l1-lib.mjs の写し。元ファイルは読み取り専用）
...
export async function launch({ shellDir, electron, project, port, isoDir }) {
    ...
    const pid = child.pid;
    try {
        return await attachLaunched({ pid, port });
    } catch (error) {
        await stop({ pid });
        throw error;
    }
...
async function attachLaunched({ pid, port }) {
    let target;
...
// 起動した Electron の子孫 PID（Helper・backend）を集める（止めた後に親が 1 へ付け替わる前に）。
function descendants(pid) {
    const found = [];
    const walk = parent => {
        const result = spawnSync('/usr/bin/pgrep', ['-P', String(parent)], { encoding: 'utf8' });
        for (const child of (result.stdout || '').split(/\s+/).filter(Boolean).map(Number)) { found.push(child); walk(child); }
    };
    walk(pid);
    return found;
}
...
export async function stop(session) {
    // 子プロセス（Helper）が残らないよう、プロセスグループごと止め、残った子孫 PID も個別に止める。
    session?.cdp?.close();
    const children = session?.pid ? descendants(session.pid) : [];
    if (session?.pid) {
        ...
        for (const child of children) { try { process.kill(child, 'SIGKILL'); } catch {} }
    }
}
