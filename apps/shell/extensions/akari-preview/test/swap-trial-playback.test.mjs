...
test('log records are one-line JSON with token, event and elapsed milliseconds', () => {
 const old = console.info, rows = []; console.info = (...args) => rows.push(args);
 try { logSwapTrial({token:'test',startedAt:Date.now()-20}, 'play_request', {attempt:1}); }
