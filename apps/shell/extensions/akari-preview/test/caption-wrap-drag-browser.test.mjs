...
test('a selected output caption keeps its grabbed edge and writes wrap width without changing size', async () => {
  ...
  try {
    ...
    await page.setContent(`<style>${baseCss}${previewSelectionHandlesStyle}
      .akari-caption__plate{position:absolute;top:var(--caption-top,auto);bottom:var(--caption-bottom,100px);
        left:var(--caption-left,100px);width:var(--caption-wrap-width,400px)}
      .akari-caption__line{width:100%;box-sizing:border-box;white-space:pre-wrap;overflow-wrap:anywhere;
    ...
    const result = await page.evaluate(() => ({ patch: window.__writes[0].plateTransform,
      ink: document.querySelector('.akari-caption__line').getBoundingClientRect().toJSON(),
    ...
    assert.equal(result.patch.cuePosition.value.anchor, 'tl');
    assert.ok(Math.abs(result.patch.cuePosition.value.position.y - (initial.y - 20) / sy / 1080) < .0001);
    assert.ok(Math.abs(result.ink.y - initial.y) < 1, 'wrap reflow keeps the visible top in place');
    assert.ok(result.ink.height > initial.height, 'the text actually reflows');
    const reloaded = await page.evaluate(({ style, widthPct }) => {
      const row = document.querySelector('.caption-row-plate');
      for (const name of ['--caption-top', '--caption-bottom', '--caption-left', '--caption-translate',
        '--caption-wrap-width']) row.style.removeProperty(name);
      for (const [name, value] of Object.entries(style)) row.style.setProperty(name, value);
      row.style.setProperty('--caption-wrap-width', widthPct + '%');
      return document.querySelector('.akari-caption__line').getBoundingClientRect().toJSON();
    }, { style: captionAnchorPositionVars(result.patch.cuePosition.value.anchor,
      result.patch.cuePosition.value.position), widthPct: result.patch.wrapWidthPct });
    assert.ok(Math.abs(reloaded.y - initial.y) < 1, 'a refresh through shared position vars keeps the top');
    assert.ok(Math.abs(reloaded.x - initial.x) < 1, 'a refresh through shared position vars keeps the left');
