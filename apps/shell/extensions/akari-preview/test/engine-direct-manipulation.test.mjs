...
test('engine cut visibility follows the timeline, not the intentionally hidden legacy video',()=>{const code=fragment('            const cutInteractionVisible =','            // frame-engine 面では本編');const run=new Function('frameEngineMediaIdle','segments','allTracksHiddenByScope','hiddenTracksByScope',`const video={style:{visibility:'hidden'}},stillImage={style:{display:'none'}},activeSegmentIndex=0;${code};return cutInteractionVisible();`);assert.equal(run(true,[{kind:'src',track:0}],{cuts:false},{cuts:new Set()}),true);assert.equal(run(true,[{kind:'gap'}],{cuts:false},{cuts:new Set()}),false);assert.equal(run(true,[{kind:'src',track:0}],{cuts:true},{cuts:new Set()}),false);assert.equal(run(false,[],{cuts:false},{cuts:new Set()}),false)});
