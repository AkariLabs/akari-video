...
test('photo selection and brush activate pointer input only while active', () => {
  ...
  const photo = previewSelectionHandlesStyle.indexOf('#preview-stage #layer-select-box.is-active.akari-photo-pointer-mode');
  assert.ok(ordinary >= 0 && photo > ordinary);
  assert.match(previewSelectionHandlesStyle.slice(photo), /#preview-stage #cut-select-box\.is-active\.akari-photo-pointer-mode \{ pointer-events: auto; \}/u);
