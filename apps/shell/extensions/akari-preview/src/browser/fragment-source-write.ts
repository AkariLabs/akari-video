...
const SESSION_ASSET_URL = /https?:\/\/(?:127\.0\.0\.1|localhost):\d+\/(?:asset|static)\//i;

interface TextSpan { start: number; end: number; value: string }

function hasSlotAttribute(tag: string, name: string): boolean {
...
function textSpans(html: string, tags: string[] = []): TextSpan[] {
    const spans: TextSpan[] = [];
    let cursor = 0;
    const slotStack: Array<{ inSlot: boolean; isSlotRoot: boolean }> = [];
...
export function patchFragmentSourceText(source: string, edited: string): string {
    const beforeTags: string[] = [];
    const afterTags: string[] = [];
