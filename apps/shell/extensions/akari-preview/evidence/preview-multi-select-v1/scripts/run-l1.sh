...
PORT=${AKARI_CDP_PORT:-9758}
# Resolve /tmp -> /private/tmp for the workspace URI containment check.
WORKSPACE=$(cd "$(mktemp -d /tmp/akari-multi-select-l1-workspace.XXXXXX)" && pwd -P)
USERDATA=$(cd "$(mktemp -d /tmp/akari-multi-select-l1-userdata.XXXXXX)" && pwd -P)
