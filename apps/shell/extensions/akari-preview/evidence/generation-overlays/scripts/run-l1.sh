#!/bin/bash
# generation-overlays L1 launcher (wrapper-authored).
# AKARI_HOME / --user-data-dir are throwaway temp dirs; only the PID started here is killed.
set -uo pipefail
REPO=${AKARI_REPO:?}
SCRIPT_DIR=$(cd "$(dirname "$0")" && pwd -P)
PORT=${AKARI_CDP_PORT:-9663}
OUT=${AKARI_OUT:?}
FFMPEG=${AKARI_FFMPEG:-ffmpeg}
WORKSPACE=$(cd "$(mktemp -d /tmp/genov-ws.XXXXXX)" && pwd -P)
USERDATA=$(cd "$(mktemp -d /tmp/genov-ud.XXXXXX)" && pwd -P)
AKARIHOME=$(cd "$(mktemp -d /tmp/genov-home.XXXXXX)" && pwd -P)
