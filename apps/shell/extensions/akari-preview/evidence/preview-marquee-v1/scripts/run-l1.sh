...
PORT=${AKARI_CDP_PORT:-9774}
# Resolve /tmp -> /private/tmp for the workspace URI containment check.
WORKSPACE=$(cd "$(mktemp -d /tmp/akari-preview-marquee-l1-workspace.XXXXXX)" && pwd -P)
USERDATA=$(cd "$(mktemp -d /tmp/akari-preview-marquee-l1-userdata.XXXXXX)" && pwd -P)
