#!/bin/bash
# caption-line-style-parity L1 launcher (wrapper-authored).
...
WORKSPACE=$(cd "$(mktemp -d /tmp/clsp-ws.XXXXXX)" && pwd -P)
USERDATA=$(cd "$(mktemp -d /tmp/clsp-ud.XXXXXX)" && pwd -P)
AKARIHOME=$(cd "$(mktemp -d /tmp/clsp-home.XXXXXX)" && pwd -P)
