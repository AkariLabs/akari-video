...
L1_MODE=${1:-${L1_MODE:-after}}
case "$L1_MODE" in before|after) ;; *) echo 'usage: run-l1.sh [before|after]' >&2; exit 2 ;; esac
export L1_MODE
PORT=${AKARI_CDP_PORT:-9737}
# Resolve /tmp -> /private/tmp for the workspace URI containment check.
WORKSPACE=$(cd "$(mktemp -d /tmp/akari-drill-in-l1-workspace.XXXXXX)" && pwd -P)
USERDATA=$(cd "$(mktemp -d /tmp/akari-drill-in-l1-userdata.XXXXXX)" && pwd -P)
...
cleanup() {
  ...
  if [ "$result" -ne 0 ] && [ "$RUNNER_STARTED" -eq 0 ]; then
    node "$SCRIPTS_DIR/run-l1.mjs" "$PORT" "$WORKSPACE" "$EVIDENCE_DIR" "$L1_MODE" --startup-failed || true
  fi
...
if [ "$READY" -ne 1 ]; then
  ...
  RUNNER_STARTED=1
  node "$SCRIPTS_DIR/run-l1.mjs" "$PORT" "$WORKSPACE" "$EVIDENCE_DIR" "$L1_MODE" --startup-failed
  exit 1
fi
RUNNER_STARTED=1
node "$SCRIPTS_DIR/run-l1.mjs" "$PORT" "$WORKSPACE" "$EVIDENCE_DIR" "$L1_MODE"
