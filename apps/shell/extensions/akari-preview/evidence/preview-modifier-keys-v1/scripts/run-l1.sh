...
L1_MODE=${1:-after}
case "$L1_MODE" in after) ;; *) echo 'usage: run-l1.sh [after]' >&2; exit 2 ;; esac
export L1_MODE
PORT=${AKARI_CDP_PORT:-9757}
# Resolve /tmp -> /private/tmp for the workspace URI containment check.
WORKSPACE=$(cd "$(mktemp -d /tmp/akari-modifier-keys-l1-workspace.XXXXXX)" && pwd -P)
USERDATA=$(cd "$(mktemp -d /tmp/akari-modifier-keys-l1-userdata.XXXXXX)" && pwd -P)
