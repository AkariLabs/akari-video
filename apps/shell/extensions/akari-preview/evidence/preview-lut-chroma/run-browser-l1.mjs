...
const allCases=[['source','a-source-chroma'],['lut100','b-lut-100'],['lut050','b-lut-050'],['layer','d-layer-chroma'],['transition',null],['inert',null],['failure',null]];
const cases=process.env.AKARI_KIND?allCases.filter(([kind])=>kind===process.env.AKARI_KIND):allCases;
const results=[];
for(const [kind,project] of cases){
  ...
  const pass=project?mad<=.01&&measured.fnv===measured.repeatFnv:kind==='transition'?measured.railCount===2&&measured.fnv===measured.repeatFnv:kind==='inert'?measured.railCount===0:measured.status==='failed'&&measured.railCount===0&&measured.badge==='LUT';
...
await writeFile(new URL('run-log.json',import.meta.url),`${JSON.stringify(report,null,2)}\n`);
