...
// L1（検証専用）: 字幕を選んだときの上のメニュー（スタイル）・下の道具バー（最小限）・「…」を
// 本物の Electron シェルで操作して、スクショと数値（edit.json / captions.json の書き込み回数・undo・DOM）を残す。
// 下地は B-1 の検証スクリプト（evidence/b1-context-bar/run-l1.mjs）。
//
// 使い方:
//   node evidence/c2-caption-context-bar/run-l1.mjs --shell <apps/shell> --label before|after --out <出力 dir>
//     [--only a,b] [--theme dark|light]
// 環境変数 AKARI_CDP_PORT（既定 9578）。一時ディレクトリの名前には 2026-09-26-libcanvas-c2-caption-context-bar を含める。
...
const repo = path.resolve(argument('repo', path.resolve(here, '../../../../../../..')));
...
const orient = argument('orient', 'landscape');
const projectName = orient;
const only = (argument('only', '') || '').split(',').filter(Boolean);
const theme = argument('theme', 'dark');
const port = Number(process.env.AKARI_CDP_PORT ?? 9623);
const electron = path.join(shellDir, 'node_modules', 'electron', 'dist', 'Electron.app', 'Contents', 'MacOS', 'Electron');

const scratch = await realpath(await mkdtemp(path.join(os.tmpdir(), `2026-09-26-preview-chrome-top-layer-${label}-${orient}-`)));
...
const scenarios = (await import(pathToFileURL(path.resolve(argument('scenarios', path.join(here, 'scenarios.mjs')))).href)).default;
...
try {
  ...
  if (view) {
    ...
    await sleep(1500);
    outer = await frameOffset();
    const api = { report: reportRef, orient, repo, captionsPath, main, browser, get view() { return view; }, get outer() { return outer; }, editPath, editUri, outDir, label, S,
    ...
    if (argument('hold')) { await writeFile('/tmp/2026-09-26-preview-chrome-top-layer-hold.json', JSON.stringify({ view, outer, editPath })); await sleep(Number(argument('hold')) * 1000); }
