...
// Verify the existing public selection getters without changing their contract.
export const EXPECTED_STATE_API = Object.freeze({
  object: 'window.akari.interaction',
  getters: ['selectedId', 'selectedIds', 'selectionKind', 'scopeId', 'floorScopeId', 'activeEdit']
});
...
async function state() {
  return pe(`(() => {
    ...
    return {selectedId:a.selectedId,selectedIds:a.selectedIds,selectionKind:a.selectionKind,
      scopeId:a.scopeId,floorScopeId:a.floorScopeId,activeEdit:Boolean(a.activeEdit)};
  })()`);
}
...
async function handlePoint(kind) {
  return waitFor(`visible ${kind} handle`, () => pe(`(() => {
    const frame=document.querySelector('.akari-interaction-selection-frame:not([hidden])');
    const handle=frame?.querySelector('.akari-interaction-handle.is-${kind}');
...
async function handleDrag(kind, dx, dy, modifiers = 1, cancel = false) {
  const from = await handlePoint(kind), to = { x: from.x + dx, y: from.y + dy };
  const index = events.length;
  await preview.send('Input.dispatchMouseEvent', {type:'mouseMoved',x:from.x,y:from.y,button:'none'});
  await preview.send('Input.dispatchMouseEvent', {type:'mousePressed',x:from.x,y:from.y,button:'left',buttons:1,clickCount:1,modifiers});
  for(let i=1;i<=8;i++) await preview.send('Input.dispatchMouseEvent',
    {type:'mouseMoved',x:from.x+dx*i/8,y:from.y+dy*i/8,button:'left',buttons:1,modifiers});
  if(cancel) await press('Escape');
  await preview.send('Input.dispatchMouseEvent', {type:'mouseReleased',x:to.x,y:to.y,button:'left',modifiers});
  if(!cancel) await waitFor(`${kind} write response`, () => events.slice(index).some(e => e.kind === 'response'));
  await sleep(700);
  return { from, to, journal: await journalSince(index) };
}
async function chooseGroup(id, leaf) {
  await resetPreview(); await seekSample();
  await clickPreviewPart(leaf);
  return expectState({ selectedId: id, selectionKind: 'group' });
}
function assertSingleWrite(gesture, id) {
  check(gesture.journal.writes.length===1&&gesture.journal.responses.length===1,
    'gesture sends one write and receives one response', gesture);
  check(gesture.journal.writes[0].overlayId===id&&gesture.journal.responses[0].ok===true,
    'gesture persisted the selected item', gesture);
  return gesture.journal.writes[0].patch.transform;
}
async function memberPoses(ids) {
  return Promise.all(ids.map(id => mountedPose(id)));
}
async function assertChildrenUnchanged(before, after, id) {
  const old=locate(before,id), next=locate(after,id);
  const oldCopy=structuredClone(before), nextCopy=structuredClone(after);
  locate(oldCopy,id).transform=structuredClone(next.transform);
  check(equal(oldCopy,nextCopy),'only the selected group item changed on disk', {id,old,next});
}

try {
  await mkdir(evidence, { recursive: true });
  if (process.argv.includes('--startup-failed')) throw new Error('Launcher setup failed or Electron readiness timed out');
  ...
  check((await diff()) === '', 'fixture must start clean');
  ...
  await waitFor('outer timeline row',
    () => evaluate(main, `Boolean(document.querySelector('[data-akari-tree-row-id="outer"]'))`), READY_MS);
  await attachPreview(); await seekSample();

  await sampleRecord(1, 'outer shows four resize handles and rotate handle', async out => {
    await chooseGroup('outer','g1.first');
    out.geometry=await geometry(['g1.first','g1.second']);
    check(out.geometry.kind==='group'&&out.geometry.visibleHandles===5&&nearRect(out.geometry.frame,out.geometry.union),
      'outer group frame has five handles and matches visible union',out.geometry);
    await shot('01-outer-handles');
  });
  await sampleRecord(2, 'outer resize scales both children and writes group once', async out => {
    await chooseGroup('outer','g1.first');
    const before=await readEdit(), liveBefore=await memberPoses(['g1.first','g1.second']);
    out.gesture=await handleDrag('se',45,30);
    const patch=assertSingleWrite(out.gesture,'outer'), after=await readEdit();
    out.liveAfter=await memberPoses(['g1.first','g1.second']);
    check(patch.scale>savedWorldTransform(before,'outer').scale,'outer scale increased',patch);
    check(out.liveAfter.every((pose,i)=>pose.scale>liveBefore[i].scale),'both child world scales increased',out);
    check(locate(after,'outer').transform.scale!==locate(before,'outer').transform.scale,'outer scale persisted');
    await assertChildrenUnchanged(before,after,'outer');
    await shot('02-outer-resized');
  });
  await sampleRecord(3, 'outer rotation snaps to 15 degrees with Shift', async out => {
    await chooseGroup('outer','g1.first');
    const before=await readEdit();
    out.gesture=await handleDrag('rotate',35,10,0);
    const first=assertSingleWrite(out.gesture,'outer');
    check(Number.isFinite(first.rotate),'outer rotation saved',first);
    out.betweenRotations=await rebuildPreview('after first outer rotation');
    await chooseGroup('outer','g1.first');
    out.shiftGesture=await handleDrag('rotate',35,10,8);
    const second=assertSingleWrite(out.shiftGesture,'outer');
    check(Math.abs((second.rotate-first.rotate)/15-Math.round((second.rotate-first.rotate)/15))<1e-6,
      'Shift rotation delta snaps to 15 degrees',{first,second});
    check(locate(await readEdit(),'outer').transform.rotate!==locate(before,'outer').transform?.rotate,'outer rotate changed on disk');
    await shot('03-outer-rotated');
  });
  await sampleRecord(4, 'bag s01 resize and rotate persist on bag only', async out => {
    const part=await partAId(); await chooseGroup('s01',part);
    const before=await readEdit();
    out.resize=await handleDrag('se',35,20);
    const resized=assertSingleWrite(out.resize,'s01');
    check(resized.scale>0&&resized.scale!==1,'bag scale changed',resized);
    await assertChildrenUnchanged(before,await readEdit(),'s01');
    out.betweenGestures=await rebuildPreview('after bag resize');
    await chooseGroup('s01',part);
    out.rotate=await handleDrag('rotate',30,15,0);
    const rotated=assertSingleWrite(out.rotate,'s01');
    check(Number.isFinite(rotated.rotate)&&rotated.rotate!==0,'bag rotate changed',rotated);
    await assertChildrenUnchanged(before,await readEdit(),'s01');
    await shot('04-bag-transformed');
  });
  await sampleRecord(5, 'plain leaf rotate writes rotate once', async out => {
    await resetPreview(); await clickPreviewPart('plain');
    await expectState({selectedId:'plain',selectionKind:'leaf'});
    const before=await readEdit();
    out.gesture=await handleDrag('rotate',30,15,0);
    const patch=assertSingleWrite(out.gesture,'plain');
    check(equal(Object.keys(patch),['rotate']),'leaf patch contains rotate only',patch);
    check(locate(await readEdit(),'plain').transform.rotate!==locate(before,'plain').transform?.rotate,'plain rotation persisted');
    await shot('05-plain-rotated');
  });
  await sampleRecord(6, 'resize leaf inside rotated g1 preserves rotation and uniform ratio', async out => {
    const doc=await readEdit(); locate(doc,'g1').transform.rotate=30;
    await writeFile(editPath,JSON.stringify(doc,null,2)+'\n');
    out.rebuild=await rebuildPreview('fixture g1 rotation setup');
    await resetPreview(); await deepSelect('g1.first');
    const before=await mountedPose('g1.first');
    out.anchorBefore=await pe(`(() => {
      const mount=[...document.querySelectorAll('#overlay-stage > [data-overlay-id]')]
        .find(e=>e.dataset.overlayId==='g1.first');
      const root=mount?.querySelector('.plain');
      if(!root)throw new Error('g1.first fixture root unavailable');
      const marker=document.createElement('span');
      marker.dataset.akariL1AnchorProbe='true';
      marker.style.cssText='position:absolute;left:0;top:0;width:0;height:0;pointer-events:none';
      root.appendChild(marker);
      const r=marker.getBoundingClientRect(); return {x:r.left,y:r.top};
    })()`);
    out.gesture=await handleDrag('se',30,18);
    const patch=assertSingleWrite(out.gesture,'g1.first');
    const after=await mountedPose('g1.first');
    check(Math.abs(after.rotate-before.rotate)<1e-4&&after.scale>before.scale,
      'rotated leaf keeps angle and scales uniformly', {before,after,patch});
    check(Math.abs(patch.rotate-before.rotate)<1e-4,'write preserves leaf world rotation',patch);
    out.anchorAfter=await pe(`(() => {
      const marker=document.querySelector('[data-akari-l1-anchor-probe="true"]');
      if(!marker)throw new Error('resize anchor marker was lost');
      const r=marker.getBoundingClientRect(); return {x:r.left,y:r.top};
    })()`);
    out.anchorDrift=Math.hypot(out.anchorAfter.x-out.anchorBefore.x,out.anchorAfter.y-out.anchorBefore.y);
    check(out.anchorDrift<=0.5,'rotated leaf opposite corner stays within 0.5 client px',
      {before:out.anchorBefore,after:out.anchorAfter,drift:out.anchorDrift});
    await shot('06-rotated-leaf-resized');
  });
  await sampleRecord(7, 'Esc during group resize restores live pose without write', async out => {
    await chooseGroup('outer','g1.first');
    const before=await readFile(editPath), poses=await memberPoses(['g1.first','g1.second']);
    out.gesture=await handleDrag('se',40,25,1,true);
    check(out.gesture.journal.writes.length===0&&out.gesture.journal.responses.length===0,'Esc sends no write',out.gesture);
    check(equal(await readFile(editPath),before),'Esc leaves edit.json byte-identical');
    out.after=await memberPoses(['g1.first','g1.second']);
    check(out.after.every((pose,i)=>poseMatches(pose,poses[i])),'Esc restores both child world poses',out);
    await shot('07-resize-cancelled');
  });
  await sampleRecord(8, 'existing group drag retains exact translation', async out => {
    await chooseGroup('outer','g1.first');
    const before=await readEdit();
    const from=await pointFor('g1.first'), index=events.length;
    // Alt disables snapping while the actual pointer travels +18/+14 client px.
    await preview.send('Input.dispatchMouseEvent',{type:'mouseMoved',x:from.x,y:from.y,button:'none'});
    await preview.send('Input.dispatchMouseEvent',{type:'mousePressed',x:from.x,y:from.y,button:'left',buttons:1,clickCount:1,modifiers:1});
    for(let i=1;i<=8;i++)await preview.send('Input.dispatchMouseEvent',
      {type:'mouseMoved',x:from.x+18*i/8,y:from.y+14*i/8,button:'left',buttons:1,modifiers:1});
    await preview.send('Input.dispatchMouseEvent',{type:'mouseReleased',x:from.x+18,y:from.y+14,button:'left',modifiers:1});
    await waitFor('group drag response',()=>events.slice(index).some(e=>e.kind==='response'));
    const journal=await journalSince(index);
    check(journal.writes.length===1&&journal.responses[0]?.ok===true&&journal.writes[0].overlayId==='outer',
      'group drag writes outer once',journal);
    const old=savedWorldTransform(before,'outer'), next=savedWorldTransform(await readEdit(),'outer');
    const display=await pe(`(() => { const s=document.getElementById('overlay-stage').getBoundingClientRect();
      return s.width/document.getElementById('overlay-stage').clientWidth; })()`);
    const dx=18/display,dy=14/display;
    check(Math.abs(next.x-old.x-dx)<1&&Math.abs(next.y-old.y-dy)<1,'group drag numeric delta unchanged',{old,next,dx,dy});
    out.observed={old,next,dx,dy,journal}; await shot('08-group-drag');
  });
