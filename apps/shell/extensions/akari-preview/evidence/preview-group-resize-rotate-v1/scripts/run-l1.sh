...
WORKSPACE=$(cd "$(mktemp -d /tmp/akari-group-resize-rotate-l1-workspace.XXXXXX)" && pwd -P)
USERDATA=$(cd "$(mktemp -d /tmp/akari-group-resize-rotate-l1-userdata.XXXXXX)" && pwd -P)
...
cleanup() {
  ...
  if [ "$result" -ne 0 ] && [ "$RUNNER_STARTED" -eq 0 ]; then
    node "$SCRIPTS_DIR/run-l1.mjs" "$PORT" "$WORKSPACE" "$EVIDENCE_DIR" after --startup-failed || true
  fi
...
if [ "$READY" -ne 1 ]; then
  ...
  RUNNER_STARTED=1
  node "$SCRIPTS_DIR/run-l1.mjs" "$PORT" "$WORKSPACE" "$EVIDENCE_DIR" after --startup-failed
  exit 1
fi
RUNNER_STARTED=1
node "$SCRIPTS_DIR/run-l1.mjs" "$PORT" "$WORKSPACE" "$EVIDENCE_DIR" after
