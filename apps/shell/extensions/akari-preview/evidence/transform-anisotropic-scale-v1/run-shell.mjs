...
const fixtures = ['scale-x', 'scale-y', 'rotated', 'group-leaf', 'keyframes', 'legacy-keyframes'];
...
function expectedBounds(fixture, frame) {
  const [sx, sy] = {
    ...
    'group-leaf': [2.5, 1.25], keyframes: [1 + frame / 60, 1],
    'legacy-keyframes': [1 + frame / 60, 1 + frame / 60],
  }[fixture];
  const angle = fixture === 'rotated' ? Math.PI / 6 : 0;
  return { width: (100 * Math.cos(angle) + 60 * Math.sin(angle)) * sx,
    height: (100 * Math.sin(angle) + 60 * Math.cos(angle)) * sy };
}
    ...
    for (const fixture of fixtures) {
      ...
      try {
        ...
        for (const frame of record.expectedFrames) {
          ...
          const sample = { frame, measured, expected, delta, pass: Math.abs(delta.width) <= 1 && Math.abs(delta.height) <= 1 };
