...
OUT=${AKARI_OUT:-/tmp/hit-region-out}
WORKSPACE=$(cd "$(mktemp -d /tmp/hit-region-ws.XXXXXX)" && pwd -P)
USERDATA=$(cd "$(mktemp -d /tmp/hit-region-ud.XXXXXX)" && pwd -P)
...
mkdir -p "$OUT"
node "$REPO/apps/shell/extensions/akari-preview/evidence/issue-36-overlay-frame/scripts/prepare-fixture.mjs" "$WORKSPACE" "$REPO" "$FIXTURE" || exit 1
...
node "$SCRIPT_DIR/run-shell-l1.mjs" "$PORT" "$WORKSPACE" "$OUT" "$LABEL"
RC=$?
