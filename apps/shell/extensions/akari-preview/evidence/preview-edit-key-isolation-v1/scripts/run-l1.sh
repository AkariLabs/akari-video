...
EVIDENCE_DIR=${AKARI_EVIDENCE_DIR:-$(CDPATH= cd -- "$SCRIPTS_DIR/.." && pwd -P)}
SHELL_DIR=$(CDPATH= cd -- "$SCRIPTS_DIR/../../../../.." && pwd -P)
PORT=${AKARI_CDP_PORT:-9777}
# Resolve /tmp -> /private/tmp for the workspace URI containment check.
WORKSPACE=$(cd "$(mktemp -d /tmp/akari-edit-key-isolation-l1-workspace.XXXXXX)" && pwd -P)
USERDATA=$(cd "$(mktemp -d /tmp/akari-edit-key-isolation-l1-userdata.XXXXXX)" && pwd -P)
