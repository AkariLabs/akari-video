...
RUN_ROOT_RAW=$(mktemp -d "${TEMP_BASE%/}/akari-alpha-matte-l1.XXXXXX")
RUN_ROOT=$(CDPATH= cd -- "$RUN_ROOT_RAW" && pwd -P)
WORKSPACE="$RUN_ROOT/workspace"
USERDATA="$RUN_ROOT/userdata"
PORT=${AKARI_L1_PORT:-9483}
ACTIVE_PID=""
...
if [ "$L1_EXIT" -ne 0 ]; then
  echo "--- electron.log tail ---"
  tail -120 "$RUN_ROOT/electron.log"
fi
exit "$L1_EXIT"
