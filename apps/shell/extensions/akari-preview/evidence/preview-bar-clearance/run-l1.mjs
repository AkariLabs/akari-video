...
// L1（検証専用）: 出力プレビューの上のメニュー（字幕・写真・図形を選んだとき）と映像の上端の間隔、
// 2 本指スクロール（ctrl なしの wheel）・ピンチ（ctrl ありの wheel）の結果を本物の Electron シェルで測る。
// 下地は C-2 の検証スクリプト（evidence/c2-caption-context-bar/run-l1.mjs）。
//
// 使い方:
//   node run-l1.mjs --shell <apps/shell> --label before|after --aspect landscape|portrait [--out <出力 dir>] [--only a,b]
// 環境変数 AKARI_CDP_PORT（既定 9628）。一時ディレクトリの名前には preview-bar-clearance を含める。
...
const aspect = argument('aspect', 'landscape');
const projectName = aspect;
const only = (argument('only', '') || '').split(',').filter(Boolean);
const theme = argument('theme', 'dark');
const port = Number(process.env.AKARI_CDP_PORT ?? 9628);
const electron = path.join(shellDir, 'node_modules', 'electron', 'dist', 'Electron.app', 'Contents', 'MacOS', 'Electron');

const scratch = await realpath(await mkdtemp(path.join(os.tmpdir(), `2026-09-26-preview-bar-clearance-${label}-${aspect}-`)));
...
await copyFile(path.join(here, 'fixtures', `${aspect}.json`), editPath);
await copyFile(path.join(here, 'fixtures', 'captions.json'), path.join(project, 'captions.json'));
...
const encoded = spawnSync(process.env.FFMPEG ?? 'ffmpeg', ['-hide_banner', '-loglevel', 'error', '-nostdin', '-y',
  '-f', 'lavfi', '-i', `color=c=0x9ca3af:s=${aspect === 'portrait' ? '1080x1920' : '1920x1080'}:d=10:r=30`, '-c:v', 'libx264', '-pix_fmt', 'yuv420p', '-g', '30',
...
for (const a of [['init', '-q'], ['config', 'user.email', 'l1@localhost'], ['config', 'user.name', 'l1'], ['add', '-A'], ['commit', '-q', '-m', 'fixture']]) {
  const g = spawnSync('/usr/bin/git', a, { cwd: project, encoding: 'utf8' });
  if (g.status !== 0) throw new Error(`git ${a[0]}: ${g.stderr}`);
}
}
const captionsPath = path.join(path.dirname(editPath), 'captions.json');
...
async function screenshot(name, clip) {
  ...
  await writeFile(path.join(outDir, `${label}-${aspect}-${name}.png`), Buffer.from(shot.data, 'base64'));
  return `${label}-${aspect}-${name}.png`;
}
...
let child;
const report = { label, port, project: projectName, aspect, theme };
try {
  ...
  if (view) {
    ...
    await sleep(1500);
    outer = await frameOffset();
    const api = { aspect, captionsPath, main, browser, get view() { return view; }, get outer() { return outer; }, editPath, editUri, outDir, label, S,
    ...
    if (argument('hold')) { await writeFile(`/tmp/2026-09-26-preview-bar-clearance-hold-${aspect}.json`, JSON.stringify({ view, outer, editPath })); await sleep(Number(argument('hold')) * 1000); }
...
await writeFile(path.join(outDir, `${label}-${aspect}.json`), `${text}\n`);
