...
const expected = (name, frame) => {
  const axes = name === 'scale-x' ? [2, 1] : name === 'scale-y' ? [1, .5] : name === 'rotated' ? [1.5, .75]
    : name === 'group-leaf' ? [2.5, 1.25] : name === 'uniform-rotated' ? [1.25, 1.25] : name === 'keyframes' ? [1 + frame / 60, 1] : name === 'legacy-keyframes' ? [1 + frame / 60, 1 + frame / 60] : [1.25, 1.25];
  const a = ['rotated', 'group-leaf', 'uniform-rotated'].includes(name) ? Math.PI / 6 : 0;
  return { width: 100 * axes[0] * Math.cos(a) + 60 * axes[1] * Math.sin(a),
    height: 100 * axes[0] * Math.sin(a) + 60 * axes[1] * Math.cos(a) };
};
