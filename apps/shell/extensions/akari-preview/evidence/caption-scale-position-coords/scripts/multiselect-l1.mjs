#!/usr/bin/env node
// 本票（caption-scale-position-coords）での再利用: akari-annotations/evidence/caption-multiselect-move/scripts/l1.mjs の写し。
// 変更点: 一時ディレクトリ名・既定ポート 9479・結果ファイル名（results-multiselect-*）・スクリーンショット名・BEFORE の参照先（D の results-before.json）・自分の Electron だけを止める pkill。
// タイムラインの複数選択 → 出力プレビューでまとめて動かす・大きさを変える の L1（ラッパー作成の検証スクリプト）。
// fixture は gen-fixture.mjs で作る。使い方: node l1.mjs <before|after> [fixture dir] [--port=9479]
...
import { S, command, launch as launchSession, sanitize, saveJson, sleep, stop as stopSession, waitEval } from './l1-lib.mjs';
async function launch(options) { const s = await launchSession(options); s.isoDir = options.isoDir; return s; }
...
const D_ROOT = path.join(REPO, 'apps', 'shell', 'extensions', 'akari-annotations', 'evidence', 'caption-multiselect-move');
...
const TMP = path.join(os.tmpdir(), 'caption-scale-position-coords-multiselect-l1');
...
const RESULTS = path.join(ROOT, `results-multiselect-${PHASE}.json`);
const STRICT = PHASE !== 'before';
const out = { phase: PHASE, status: 'running', checks: [], screenshots: [] };
// AFTER の回帰比較に使う BEFORE の記録（無ければ比較は null）。
const BEFORE = STRICT ? await readFile(path.join(D_ROOT, 'results-before.json'), 'utf8').then(JSON.parse).catch(() => null) : null;
const META = 4, ALT = 1;
const DRAG_PX = 40;
...
async function shot(session, name, clip) {
    await sleep(500);
    const file = `${PHASE}-multiselect-${name}.png`;
