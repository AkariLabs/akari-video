#!/usr/bin/env node
// 本票での再利用: akari-annotations/evidence/caption-multiselect-move/scripts/gen-fixture.mjs の写し（出力先の名前だけ変更）。
// タイムラインの複数選択 → 出力プレビューでまとめて動かす L1 の fixture（ラッパー作成の検証用素材）。
...
const OUT = path.resolve(process.argv[2] ?? path.join(os.tmpdir(), 'caption-scale-position-coords-multiselect-l1', 'fixture'));
...
run('/usr/bin/git', ['config', 'user.email', 'cmm-fixture@localhost'], dir);
run('/usr/bin/git', ['config', 'user.name', 'cmm fixture'], dir);
