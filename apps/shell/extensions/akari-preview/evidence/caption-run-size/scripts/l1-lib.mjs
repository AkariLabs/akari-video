// L1 の共通部品（caption-drag-and-icon-tools/scripts/l1-lib.mjs の写し + 起動前の prepare・ライブラリの置き場。元ファイルは読み取り専用）
...
export async function launch({ shellDir, electron, project, port, isoDir, prepare }) {
    await rm(isoDir, { recursive: true, force: true });
    await mkdir(isoDir, { recursive: true });
    if (prepare) await prepare(isoDir);
    ...
    // 画面の大きさの上書き（Emulation）はしない: 上書きした状態では出力プレビューの webview へのクリックで字幕が選ばれなかった（本票の L1 で実測）
    if (process.env.L1_EMULATE) await cdp.send('Emulation.setDeviceMetricsOverride', { width: 1440, height: 900, deviceScaleFactor: 1, mobile: false }).catch(() => {});
