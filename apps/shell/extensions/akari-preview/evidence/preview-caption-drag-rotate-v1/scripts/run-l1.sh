...
export L1_MODE
PORT=${AKARI_CDP_PORT:-9768}
# Resolve /tmp -> /private/tmp for the workspace URI containment check.
WORKSPACE=$(cd "$(mktemp -d /tmp/akari-caption-drag-rotate-l1-workspace.XXXXXX)" && pwd -P)
USERDATA=$(cd "$(mktemp -d /tmp/akari-caption-drag-rotate-l1-userdata.XXXXXX)" && pwd -P)
