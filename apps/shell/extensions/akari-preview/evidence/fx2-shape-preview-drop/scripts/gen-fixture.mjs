#!/usr/bin/env node
// P-3 の L1 の fixture（P-1 の写し + 30 秒・0:10〜0:15 の空のキャンバス）。出力先はリポの外（引数 1 つ目。既定は OS の一時ディレクトリ）。
...
const OUT = path.resolve(process.argv[2] ?? path.join(os.tmpdir(), 'ptb-l1', 'fixture'));
...
async function project(name, withCaptions) {
    ...
    // 0:10〜0:15 の空のキャンバス（「キャンバスを作る」と同じ形。字幕の袋と重なるので字幕の段の上の段）
    tracks.push({ id: 'v-canvas', lane: 'visual', name: 'V1', items: [{
        id: 'g-1', name: 'キャンバス 1', at: 10 * FPS, duration: 5 * FPS, source: { kind: 'group', canvas: { origin: 'user', durationMode: 'fixed' } }, items: []
    }] });
    ...
    run('/usr/bin/git', ['config', 'user.email', 'ptb-fixture@localhost'], dir);
    run('/usr/bin/git', ['config', 'user.name', 'ptb fixture'], dir);
    run('/usr/bin/git', ['add', '-A'], dir);
    run('/usr/bin/git', ['commit', '-q', '-m', 'fixture'], dir);
    return dir;
}
console.log(JSON.stringify({ spoken: await project('spoken', true) }));
